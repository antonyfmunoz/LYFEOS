...
| LFE-15 | AI can assist safely through governed action | Metadata-only action receipts, visible Profile activity, privacy-safe previews, a 15-minute explicit approval queue for medium-risk actions, and historical planning-context snapshots on every recorded action. The AI prompt receives current decision context only through existing privacy preferences; mission mutations use the canonical lifecycle. | **Implemented foundation, local** | Add action-specific rollback/repair, richer policy tiers, authenticated approval tests, and deployed AI-provider qualification. |
