...
- 136 Postgres tables in `shared/schema.ts`.
- 56 client routes in `client/src/App.tsx`.
- 463 unit/integration test declarations across 96 test files.
- 101 SQL migrations, including Threads, evidence, skill progression, badges, governed AI actions, cross-product sharing, Health, instruments, Projects, Messages, and transformation intelligence.
...
| LFE-27 | Reliable release process | migrations, `npm run verify`, Docker/Fly configuration, GitHub Actions; the migration set contains 101 migrations from `0000` through `0100`, with the release runner carrying 92 migrations from `0009` through `0100`. On 2026-08-24, a clean PostgreSQL 16 container applied all 101 SQL files, exposed 137 public tables, accepted all 92 release-runner entries through `0100_transformation_intelligence`, and produced an exact zero-output second-run no-op. A production build then completed a six-test authenticated transformation journey across two temporary accounts: Thread-bound context/calibration capture, versioned weighted proof, exact-rubric human revision, appeal, reconsideration with real capability XP, evidence withdrawal/reversal, and account cleanup. The required PostgreSQL CI job now carries both Messages and transformation journeys; its definition is locally tested, but no remote result exists until this revision is pushed. | **Local migration and authenticated API gate complete; CI enforcement defined** | Obtain remote CI, rehearse forward-only rollback in a managed environment, then obtain immutable-source deployment, production migration, browser/device, and deployment receipts. |
