...
Capture stays unavailable unless all five values are configured and a live provider preflight verifies the expected numeric project ID and public project token, client-IP discard, disabled autocapture/web-vitals/console/performance/session-recording/heatmap/dead-click capture, and the explicitly enforced approved three-month retention period:
...
## Provider control-plane state (2026-08-30)

The dedicated PostHog project exists, but production capture remains intentionally unavailable. A current authenticated provider read on 2026-08-30 found that the project still permits automatic capture, client IP retention, web-vitals/console/performance capture, session recording, and heatmaps; raw-event retention is 84 months and is not explicitly enforced. Those provider defaults conflict with this contract even though the LyfeOS SDK also disables the corresponding browser features. The provider reports that no LyfeOS event has ever been ingested, and Fly still does not hold the five required PostHog values. Do not configure only the public project token: provider-side deletion requires the server-only project ID, personal API key, and admin host as one complete activation unit. Installing all five values is still insufficient until the server preflight passes.

Exact source `6c734287c70fae4ba265266b743dc007f2d0d04c` passed protected CI `33306961512` and deployed as Fly release 182. Its server preflight verifies the exact numeric project ID and public token in addition to every privacy and retention control above. Protected production run `33307285639` passed the complete application suite while analytics remained off, which proves the app stays functional when the provider contract is unavailable; it is not analytics-event or provider-deletion evidence.
...
For public MVP, use an explicitly enforced **three-month raw-event retention period**. That is long enough to measure the first activation and short-retention cohorts while avoiding the current seven-year default. Revisit it only through a documented analytics need, access review, privacy-language update, and legal review.

PostHog currently derives the raw-event retention field from the organization's billing entitlement and exposes it read-only through the project API. If the LYFEOS account cannot obtain an enforced three-month entitlement, do not weaken this control or activate collection by substituting an unapproved period. Keep analytics unavailable until the provider can enforce the approved window or the operating decision is deliberately revised with the same privacy and legal review.

Activation is complete only after an authorized project administrator disables autocapture, web-vitals, session recording, console/performance capture, heatmaps, and dead-click capture; enables client-IP discard; enforces the three-month retention period; creates a personal API key limited to `project:read`, `person:read`, and `person:write`; installs all five Fly secrets together; approves the privacy language; opts in a test account; observes one allowed event and no blocked content/properties; withdraws consent; and retains the accepted person/event/recording deletion receipt. If any step fails, remove the five secrets together and keep capture unavailable.
