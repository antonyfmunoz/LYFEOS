...
const nutritionDiary = fs.readFileSync("client/src/components/health/NutritionDiary.tsx", "utf8");
...
describe("production Health offline browser acceptance custody", () => {
  it("binds a disposable exact-source desktop/mobile journey into protected production acceptance", () => {
    ...
    expect(script).toContain('new URL("/health-log", BASE_URL)');
  ...
  it("proves failure refusal, device-only queueing, reconnect, exact-once persistence, reload, and truthful boundaries", () => {
    ...
    expect(script).toContain("offlineNutritionRenderedAsDeviceOnly");
    expect(script).toContain("nutritionReconnectSyncedExactlyOnce");
    expect(script).toContain("reloadRenderedPersistedNutrition");
    expect(script).toContain("createAcceptanceFood");
    expect(script).toContain("waitForNutritionCount");
  ...
  it("uses stable semantic hooks without changing the Health layout", () => {
    ...
    expect(dailyLog).toContain('data-testid="health-supplement-name"');
    expect(dailyLog).toContain('data-testid="health-supplement-amount"');
    expect(dailyLog).toContain('data-testid="health-supplement-unit"');
    ...
    expect(metricsLedger).toContain('data-testid="health-metrics-ledger"');
    expect(metricsLedger).toContain('data-testid="health-observation-name"');
