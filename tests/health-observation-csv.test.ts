...
describe("reviewed health CSV import", () => {
  it("parses quoted rows without inferring missing health data", () => {
    ...
    expect(preview.rows[0].entry).toMatchObject({ source: "lab", sourceRecordId: "lab-1", metricKey: "ldl", value: 120, labName: "Example Lab" });
  });
  ...
  it("uses a reviewed category fallback and Test Name only when the export supplies no separate display metadata", () => {
    ...
    expect(preview.validCount).toBe(1);
    expect(preview.rows[0].entry).toMatchObject({ source: "lab", sourceRecordId: "acc-1::hdl_cholesterol", metricKey: "hdl_cholesterol", displayName: "HDL Cholesterol", category: "lab" });
  });
