...
describe("production food-catalog acceptance custody", () => {
  it("requires an immutable-release, disposable-account proof of attributed external catalog behavior", () => {
    ...
    expect(script).toContain("catalog receipt was not rejected");
    ...
    expect(script).toContain("for (const provider of providers)");
    expect(script).toContain("providerCount: qualifiedProviders.length");
    expect(script).toContain('contract: "lyfeos.production-food-catalog.v2"');
