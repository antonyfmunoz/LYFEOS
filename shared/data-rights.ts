...
export const LYFEOS_DATA_CLASSES: readonly LyfeOSDataClass[] = [
  ...
  },
  {
    ...
    examples: ["connection status", "revisioned consent scopes", "sync state", "UMH commands", "outbox receipts", "work links", "signed extension installation and permission receipts"],
    access: "restricted_operations",
    retention: "until_owner_deletes",
    retentionDetail: "Current connection state and append-only local consent/reconciliation receipts remain until account deletion; revocation clears active scope and queues a signed state change when a receiver is configured. Provider-side records follow provider policy.",
    rights: { export: true, erase: true, revoke: true },
    rightsDetail: "The owner can revoke or disconnect supported connections and signed extensions. Ecosystem consent revisions and safe outbox receipts are portable, and account erasure removes their LyfeOS-side copies. Extension installation permissions and audits are portable; publisher keys, package signatures, tokens, opaque cursors, and signing credentials are not personal export data. Remote copies remain governed by the receiving provider.",
  },
  {
