...
type BridgeDevice = { id: string; displayName: string; platform: "macos" | "android"; status: string; permissions: { read?: boolean; send?: boolean } };
type Conversation = {
  id: string;
  title: string;
  kind: "direct" | "group";
...
function channelLabel(provider?: string) {
  ...
  if (provider === "android_sms_bridge") return "Phone text";
...
function MessageChannelStamp({ provider, direction }: Pick<Message, "provider" | "direction">) {
  ...
  const Icon = provider === "imessage_bridge" || provider === "android_sms_bridge" ? Smartphone : MessageCircle;
...
export default function MessagesPage() {
  ...
  const [newConversationChannel, setNewConversationChannel] = useState<"lyfeos" | "imessage" | "sms">("lyfeos");
  ...
  const activeIMessageBridge = (bridgeStatus.data?.devices || []).find((device) => device.platform === "macos" && device.status === "active" && device.permissions.send);
  const activeAndroidBridge = (bridgeStatus.data?.devices || []).find((device) => device.platform === "android" && device.status === "active" && device.permissions.send);
  ...
  const createBridgeConversation = useMutation({
    mutationFn: () => {
      const device = newConversationChannel === "sms" ? activeAndroidBridge : activeIMessageBridge;
      if (!device) throw new Error(newConversationChannel === "sms" ? "Pair an Android phone with sending enabled in Connections before starting a phone-text conversation." : "Pair a Mac with sending enabled in Connections before starting an iMessage conversation.");
      return apiRequest<{ conversation: Conversation; created: boolean; attached: boolean }>("/api/message-bridge/conversations", { method: "POST", body: JSON.stringify({ deviceId: device.id, channel: newConversationChannel, recipientHandle: bridgeRecipient, conversationId: selectedId }) });
    },
    onSuccess: async ({ conversation, created, attached }) => {
      setBridgeRecipient(""); setStatus("open");
      await invalidate(conversation.id); setSelectedId(conversation.id);
      const name = newConversationChannel === "sms" ? "Phone text" : "iMessage";
      toast({ title: attached ? `${name} added to this conversation` : created ? `${name} conversation ready` : "Opened existing bridged conversation" });
    },
    onError: (error) => toast({ title: "Bridged conversation was not opened", description: error instanceof Error ? error.message : "Try again.", variant: "destructive" }),
  });
  ...
  const isDeviceBridgeConversation = selectedBinding?.provider === "imessage_bridge" || selectedBinding?.provider === "android_sms_bridge";
  useEffect(() => {
  ...
  return (
    <div className="container max-w-7xl space-y-4 py-5">
      ...
      <header className="flex flex-wrap items-start justify-between gap-3">
        <div>
          <p className="text-xs font-mono uppercase tracking-[0.14em] text-primary">Private unified communication</p>
          <h1 className="font-orbitron text-2xl">Messages</h1>
          <p className="max-w-3xl text-sm text-muted-foreground">One private inbox for LyfeOS conversations and explicitly paired channels. Health, missions, memories, AI history, and private Rolodex context are never inserted into a message automatically.</p>
        </div>
        <div className="flex items-center gap-2 rounded-lg border border-primary/15 bg-card/35 px-3 py-2 text-xs text-muted-foreground"><LockKeyhole className="h-4 w-4 text-primary" />Private channels only</div>
      </header>
      ...
      <section className="rounded-xl border border-primary/15 bg-card/35 p-3">
        <div className="mb-3 flex flex-wrap gap-2" aria-label="New conversation channel">
          ...
          <button type="button" onClick={() => setNewConversationChannel("sms")} className={`rounded-full border px-3 py-1.5 text-xs ${newConversationChannel === "sms" ? "border-primary bg-primary/15 text-primary" : "border-primary/15 text-muted-foreground hover:bg-primary/5"}`}>Phone text</button>
        </div>
        {newConversationChannel === "lyfeos" ? <>
          ...
          <div className="grid gap-2 md:grid-cols-[minmax(220px,1fr)_auto]"><Input aria-label={newConversationChannel === "sms" ? "Phone number" : "iMessage phone number or email"} value={bridgeRecipient} onChange={(event) => setBridgeRecipient(event.target.value)} placeholder={newConversationChannel === "sms" ? "Phone number…" : "iMessage phone number or email…"} /><Button onClick={() => createBridgeConversation.mutate()} disabled={!bridgeRecipient.trim() || !(newConversationChannel === "sms" ? activeAndroidBridge : activeIMessageBridge) || createBridgeConversation.isPending}><Plus className="mr-1 h-4 w-4" />{selectedId ? newConversationChannel === "sms" ? "Add phone text" : "Add iMessage" : newConversationChannel === "sms" ? "New phone text" : "New iMessage"}</Button></div>
          <p className="text-xs text-muted-foreground">{newConversationChannel === "sms" ? activeAndroidBridge ? selectedId ? "This adds phone texting to the current conversation. Every channel will share one history." : `New conversations will be sent through ${activeAndroidBridge.displayName} after you type the first message.` : "Pair an Android phone with sending enabled in Connections to start a phone-text conversation. RCS remains unavailable unless Android provides that capability to the bridge." : activeIMessageBridge ? selectedId ? "This adds iMessage to the current conversation. Every channel will share one history." : `New conversations will be sent through ${activeIMessageBridge.displayName} after you type the first message.` : "Pair a Mac with sending enabled in Connections to start an iMessage conversation."}</p>
        </div>}
      </section>
      ...
      <div className="grid min-h-[560px] overflow-hidden rounded-xl border border-primary/15 bg-card/25 lg:grid-cols-[320px_1fr]">
        <aside className="border-b border-primary/15 p-2 lg:border-b-0 lg:border-r">
          {list.isPending ? <div role="status" className="p-8 text-center text-sm text-muted-foreground">Loading conversations…</div> : list.isError ? <div role="alert" className="space-y-3 p-8 text-center text-sm text-muted-foreground"><p>Conversations could not be loaded.</p><Button size="sm" variant="outline" onClick={() => list.refetch()} disabled={list.isFetching}>{list.isFetching ? "Retrying…" : "Retry"}</Button></div> : list.data?.conversations.length ? list.data.conversations.map((item) => {
            const others = item.participants.filter((participant) => participant.id !== user?.id);
            return <button key={item.id} data-testid={`messages-conversation-list-${item.id}`} onClick={() => setSelectedId(item.id)} className={`mb-1 w-full rounded-lg border p-3 text-left ${selectedId === item.id ? "border-primary/40 bg-primary/10" : "border-transparent hover:bg-primary/5"}`}>
              <div className="flex items-start gap-3"><div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-primary/25 bg-primary/10 text-xs text-primary">{item.kind === "group" ? <Users className="h-4 w-4" /> : initials(others[0]?.displayName || item.title)}</div><div className="min-w-0 flex-1"><div className="flex items-center justify-between gap-2"><strong className="truncate text-sm">{conversationLabel(item, user?.id)}</strong><span className="text-[10px] text-muted-foreground">{relativeTime(item.latestMessage?.createdAt || item.lastMessageAt)}</span></div><div className="mt-0.5 flex min-w-0 items-center gap-1.5"><span className="shrink-0 rounded border border-primary/15 px-1.5 py-0.5 text-[9px] font-mono text-muted-foreground">{channelLabel(item.bindings?.find((binding) => binding.status === "active")?.provider)}</span><p className="truncate text-xs text-muted-foreground">{item.latestMessage ? `${item.latestMessage.direction === "outbound" ? "You: " : ""}${item.latestMessage.body}` : others.map((person) => person.displayName).join(", ")}</p></div></div>{item.unreadCount > 0 && <span className="rounded-full bg-primary px-1.5 py-0.5 text-[10px] text-primary-foreground">{item.unreadCount}</span>}</div>
            </button>;
        ...
        {conversation ? <section aria-label="Selected conversation" data-testid={`messages-conversation-${conversation.id}`} className="flex min-h-[560px] flex-col">
          <div className="flex flex-wrap items-center justify-between gap-2 border-b border-primary/15 p-4"><div><div className="flex items-center gap-2"><MessageCircle className="h-4 w-4 text-primary" /><strong>{conversationLabel(conversation, user?.id)}</strong>{(conversation.bindings || []).filter((binding) => binding.status === "active").map((binding) => <span key={binding.id} className="rounded border border-primary/15 px-1.5 py-0.5 text-[9px] font-mono text-muted-foreground">{channelLabel(binding.provider)}</span>)}</div><p className="mt-1 text-xs text-muted-foreground">{conversation.participants.length > 1 ? `${conversation.participants.map((person) => person.displayName).join(", ")} · ` : ""}{labels[conversation.status]} · revision {conversation.version}</p></div><div className="flex flex-wrap gap-1">
          ...
          {conversation.kind === "group" && conversation.participantStatus === "active" && <details className="border-b border-primary/15 bg-background/20 px-4 py-2">
            ...
            <div className="mt-3 space-y-2">
              {canManageGroup && <div><Input aria-label="Find a group participant" value={groupMemberSearch} onChange={(event) => setGroupMemberSearch(event.target.value)} placeholder="Add a LyfeOS username…" />{groupMemberSearch.trim().length >= 2 && <div className="mt-1 flex flex-wrap gap-1">{(groupPeople.data?.users || []).filter((person) => !conversation.participants.some((participant) => participant.id === person.id)).map((person) => <Button key={person.id} size="sm" variant="outline" onClick={() => addGroupParticipant.mutate(person.id)}>+ {person.displayName}</Button>)}</div>}</div>}
              ...
              return <div key={message.id} data-message-id={message.id} data-testid={`native-message-${message.id}`} className={`group flex ${outbound ? "justify-end" : "justify-start"}`}>
                <div className={`max-w-[82%] rounded-2xl border px-3 py-2 ${outbound ? "border-primary/30 bg-primary/12" : "border-primary/15 bg-background/55"}`}>
                  ...
                  <div className="mt-1 flex items-center justify-end gap-2">
                    {!deleted && canCompose && <><div className="flex gap-0.5 opacity-0 transition-opacity group-hover:opacity-100">{nativeMessageReactions.map((reaction) => <button key={reaction} type="button" aria-label={`React ${reaction}`} data-testid={`native-message-react-${message.id}-${reaction}`} className="rounded px-1 text-xs hover:bg-primary/10" onClick={() => reactToMessage.mutate({ messageId: message.id, reaction })}>{reaction}</button>)}</div><button aria-label="Reply to message" data-testid={`native-message-reply-${message.id}`} className="text-[10px] text-muted-foreground opacity-0 hover:text-primary group-hover:opacity-100" onClick={() => { setReplyTo(message); setNoteMode(false); }}><Reply className="inline h-3 w-3" /> reply</button></>}
                    {!deleted && outbound && canCompose && <><button aria-label="Edit message" data-testid={`native-message-edit-${message.id}`} className="text-muted-foreground opacity-0 hover:text-primary group-hover:opacity-100" onClick={() => { setEditingMessageId(message.id); setEditBody(message.body); }}><Pencil className="h-3 w-3" /></button><button aria-label="Delete message" data-testid={`native-message-delete-${message.id}`} className="text-muted-foreground opacity-0 hover:text-destructive group-hover:opacity-100" onClick={() => { if (window.confirm("Delete this message? Its attachments will no longer be shared.")) deleteMessage.mutate({ messageId: message.id, expectedVersion: message.version }); }}><Trash2 className="h-3 w-3" /></button></>}
