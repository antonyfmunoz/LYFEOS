...
export default function SpreadsheetEditorPage() {
  ...
  const insertAxis = (axis: "row" | "column") => {
    try {
      const insertionIndex = axis === "row" ? selectedPosition.row : selectedPosition.column;
      const shifted = insertSpreadsheetDocumentAxis(document, activeSheet.id, axis, insertionIndex);
      updateDocument(shiftSpreadsheetChartsForAxis(shifted, activeSheet.id, axis, insertionIndex));
  ...
  return <div data-testid="sheet-editor" className="container max-w-[1500px] py-5 space-y-4">
    ...
    <div className="rounded-xl border border-primary/15 bg-card/30 overflow-hidden">
      ...
      <div className="flex flex-wrap items-center gap-1 border-t border-primary/15 bg-background/40 p-2">
        {document.sheets.map((sheet) => <button key={sheet.id} type="button" aria-label={`Open sheet tab ${sheet.name}`} aria-pressed={sheet.id === activeSheet.id} onClick={() => { if (sheet.id !== activeSheet.id) updateDocument({ ...document, activeSheetId: sheet.id }); setSelectedAddress("A1"); setRangeAnchor("A1"); setRangeEnd("A1"); setExtendSelection(false); }} className={`max-w-40 truncate rounded px-3 py-1 text-xs ${sheet.id === activeSheet.id ? "bg-primary/20 text-primary" : "text-muted-foreground hover:bg-primary/10"}`} title={sheet.name}>{sheet.name}</button>)}
        ...
        <div className="ml-auto flex min-w-0 items-center gap-1">
          ...
          <Button type="button" size="sm" variant="outline" className="h-7" aria-label="Rename active sheet" disabled={!sheetNameDraft.trim() || sheetNameDraft.trim() === activeSheet.name} onClick={renameActiveSheet}>Rename</Button>
