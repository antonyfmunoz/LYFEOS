...
export default function DashboardPage() {
  ...
  // State for date, time, and settings
  ...
  const [widgets, setWidgets] = useState<WidgetData[]>(() => {
    ...
    const savedWidgets = localStorage.getItem('dashboardWidgetOrder');
    if (savedWidgets) {
      try {
        // Parse saved widget IDs
        const widgetOrder = JSON.parse(savedWidgets);
        
        // Create the ordered widget array
        const orderedWidgets = [
          {
            id: widgetOrder[0] || uuidv4(),
            title: "Stats Log",
    ...
    // Default widget order if no saved order exists
    return [
      {
        id: uuidv4(),
        title: "Stats Log",
  ...
  useEffect(() => {
    ...
    localStorage.setItem('dashboardWidgetOrder', JSON.stringify(widgetIds));
  }, [widgets]);
