  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-8" aria-labelledby="finance-title" data-testid="personal-finance">
      ...
      <div className="mb-6 flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <label className="flex items-center gap-2 text-sm text-muted-foreground"><span>Month</span><MonthPicker ariaLabel="Finance month" dataTestId="finance-month-picker" value={month} onChange={setMonth} className="w-40" /></label>
      </div>
      ...
      {summaryQuery.isLoading ? <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div> : summaryQuery.isError || !data ? <div className="rounded-xl border border-destructive/40 p-6"><p>Personal finance could not load.</p><Button className="mt-4" onClick={() => summaryQuery.refetch()}><RefreshCcw className="mr-2 h-4 w-4" />Retry</Button></div> : <>
        <section aria-label="Net worth and cash flow" className="mb-6 grid gap-4 md:grid-cols-3" data-testid="finance-summary">
        ...
        <section className="mb-6 grid gap-6 lg:grid-cols-2">
          <div className="glassmorphic rounded-xl border border-primary/25 p-5"><h2 className="mb-4 flex items-center gap-2 font-orbitron text-lg text-primary"><WalletCards className="h-5 w-5" />Accounts</h2><form onSubmit={createAccount} className="grid gap-3 sm:grid-cols-2" data-testid="finance-account-form"><Input required aria-label="Account name" placeholder="Account name" value={account.name} onChange={(event) => setAccount({ ...account, name: event.target.value })} className={fieldClass} /><select aria-label="Account type" value={account.accountType} onChange={(event) => setAccount({ ...account, accountType: event.target.value })} className={selectClass}>{["checking","savings","cash","investment","property","credit","loan","other_asset","other_liability"].map((value) => <option key={value} value={value}>{value.replace("_", " ")}</option>)}</select><Input required inputMode="decimal" aria-label="Current balance or amount owed" placeholder="Current balance / amount owed" value={account.balance} onChange={(event) => setAccount({ ...account, balance: event.target.value })} className={fieldClass} /><Input required maxLength={3} aria-label="Currency" value={account.currency} onChange={(event) => setAccount({ ...account, currency: event.target.value.toUpperCase() })} className={fieldClass} /><Button disabled={mutation.isPending} className="sm:col-span-2" data-testid="finance-add-account"><Plus className="mr-2 h-4 w-4" />Add account</Button></form><div className="mt-5 space-y-3">{data.accounts.map((item) => <div key={item.id} className="rounded-lg border border-muted/20 bg-background/30 p-3" data-testid={`finance-account-${item.id}`}><div className="flex items-center justify-between gap-3"><div><p className="font-medium">{item.name}</p><p className="text-xs text-muted-foreground">{item.accountType.replace("_", " ")} · {item.status} · v{item.version}</p></div><strong className="text-primary">{money(item.balanceMinor, item.currency)}</strong></div><div className="mt-3 flex flex-wrap gap-2">{item.status === "active" && <><Input inputMode="decimal" aria-label={`New balance for ${item.name}`} placeholder="Updated balance" value={balanceDrafts[item.id] ?? ""} onChange={(event) => setBalanceDrafts({ ...balanceDrafts, [item.id]: event.target.value })} className={`${fieldClass} h-9 min-w-32 flex-1`} /><Button type="button" variant="outline" size="sm" aria-label={`Update balance for ${item.name}`} onClick={() => updateBalance(item)}>Update</Button></>}<Button type="button" variant="ghost" size="sm" aria-label={`${item.status === "active" ? "Close" : "Reopen"} ${item.name}`} onClick={() => setAccountStatus(item, item.status === "active" ? "closed" : "active")}>{item.status === "active" ? <><Archive className="mr-2 h-4 w-4" />Close</> : <><RotateCcw className="mr-2 h-4 w-4" />Reopen</>}</Button></div></div>)}</div></div>

          <div className="glassmorphic rounded-xl border border-primary/25 p-5"><h2 className="mb-4 font-orbitron text-lg text-primary">Cash flow</h2>{editingTransaction && <div className="mb-3 flex items-center justify-between rounded-md border border-primary/30 bg-primary/10 p-2 text-xs"><span>Correcting {editingTransaction.description}</span><Button type="button" variant="ghost" size="sm" onClick={cancelTransactionEdit}><X className="mr-1 h-4 w-4" />Cancel</Button></div>}<form onSubmit={createTransaction} className="grid gap-3 sm:grid-cols-2" data-testid="finance-transaction-form"><select required disabled={Boolean(editingTransaction)} aria-label="Transaction account" value={transaction.accountId} onChange={(event) => setTransaction({ ...transaction, accountId: event.target.value })} className={selectClass}><option value="">Choose account</option>{data.accounts.map((item) => <option key={item.id} value={item.id}>{item.name} ({item.currency})</option>)}</select><select aria-label="Income or expense" value={transaction.direction} onChange={(event) => setTransaction({ ...transaction, direction: event.target.value })} className={selectClass}><option value="expense">Expense</option><option value="income">Income</option></select><Input required inputMode="decimal" aria-label="Transaction amount" placeholder="Amount" value={transaction.amount} onChange={(event) => setTransaction({ ...transaction, amount: event.target.value })} className={fieldClass} /><DatePicker ariaLabel="Transaction date" dataTestId="finance-transaction-date" value={transaction.transactionDate} onChange={(transactionDate) => setTransaction({ ...transaction, transactionDate })} className={fieldClass} /><Input required aria-label="Description" placeholder="Description" value={transaction.description} onChange={(event) => setTransaction({ ...transaction, description: event.target.value })} className={fieldClass} /><Input required aria-label="Category" placeholder="Category" value={transaction.category} onChange={(event) => setTransaction({ ...transaction, category: event.target.value })} className={fieldClass} /><Button disabled={mutation.isPending || (!editingTransaction && !activeAccounts.length)} className="sm:col-span-2" data-testid="finance-save-transaction">{editingTransaction ? <><Check className="mr-2 h-4 w-4" />Save correction</> : <><Plus className="mr-2 h-4 w-4" />Add transaction</>}</Button></form><div className="mt-5 max-h-80 space-y-2 overflow-auto">{data.transactions.map((item) => <div key={item.id} className="flex items-center justify-between gap-3 rounded-lg border border-muted/20 bg-background/30 p-3" data-testid={`finance-transaction-${item.id}`}><div className="min-w-0"><p className="truncate text-sm font-medium">{item.description}</p><p className="text-xs text-muted-foreground">{item.transactionDate} · {item.category}</p></div><div className="flex items-center gap-1"><strong className={item.amountMinor < 0 ? "mr-1 text-foreground" : "mr-1 text-primary"}>{money(item.amountMinor, item.currency)}</strong>{item.source === "manual" && <><Button type="button" variant="ghost" size="icon" aria-label={`Correct ${item.description}`} onClick={() => editTransaction(item)}><Pencil className="h-4 w-4" /></Button><Button type="button" variant="ghost" size="icon" aria-label={`Delete ${item.description}`} onClick={() => deleteTransaction(item)}><Trash2 className="h-4 w-4" /></Button></>}</div></div>)}</div></div>
        </section>

        <section className="grid gap-6 lg:grid-cols-2">
          <div className="glassmorphic rounded-xl border border-primary/25 p-5"><h2 className="mb-4 font-orbitron text-lg text-primary">Monthly budgets</h2><form onSubmit={saveBudget} className="grid gap-3 sm:grid-cols-3" data-testid="finance-budget-form"><Input required aria-label="Budget category" placeholder="Category" value={budget.category} onChange={(event) => setBudget({ ...budget, category: event.target.value })} className={fieldClass} /><Input required inputMode="decimal" aria-label="Budget limit" placeholder="Limit" value={budget.limit} onChange={(event) => setBudget({ ...budget, limit: event.target.value })} className={fieldClass} /><Input required maxLength={3} aria-label="Budget currency" value={budget.currency} onChange={(event) => setBudget({ ...budget, currency: event.target.value.toUpperCase() })} className={fieldClass} /><Button disabled={mutation.isPending} className="sm:col-span-3">Save budget</Button></form><div className="mt-5 space-y-3">{data.budgets.map((item) => { const pct = Math.min(100, Math.round(item.spentMinor / item.limitMinor * 100)); return <div key={item.id} data-testid={`finance-budget-${item.id}`}><div className="flex items-center justify-between gap-2 text-sm"><span>{item.category}</span><div className="flex items-center"><span>{money(item.spentMinor, item.currency)} / {money(item.limitMinor, item.currency)}</span><Button type="button" variant="ghost" size="icon" aria-label={`Delete ${item.category} budget`} onClick={() => deleteBudget(item)}><Trash2 className="h-4 w-4" /></Button></div></div><div className="mt-1 h-2 overflow-hidden rounded bg-muted/20"><div className="h-full bg-primary" style={{ width: `${pct}%` }} /></div></div>; })}</div></div>

          <div className="glassmorphic rounded-xl border border-primary/25 p-5"><h2 className="mb-4 flex items-center gap-2 font-orbitron text-lg text-primary"><Target className="h-5 w-5" />Financial goals</h2><form onSubmit={createGoal} className="grid gap-3 sm:grid-cols-2" data-testid="finance-goal-form"><Input required aria-label="Goal name" placeholder="Goal name" value={goal.name} onChange={(event) => setGoal({ ...goal, name: event.target.value })} className={fieldClass} /><select aria-label="Goal type" value={goal.goalType} onChange={(event) => setGoal({ ...goal, goalType: event.target.value })} className={selectClass}>{["savings","emergency_fund","debt_paydown","net_worth","other"].map((value) => <option key={value} value={value}>{value.replace("_", " ")}</option>)}</select><Input required inputMode="decimal" aria-label="Goal target" placeholder="Target" value={goal.target} onChange={(event) => setGoal({ ...goal, target: event.target.value })} className={fieldClass} /><Input inputMode="decimal" aria-label="Goal current progress" placeholder="Current progress" value={goal.current} onChange={(event) => setGoal({ ...goal, current: event.target.value })} className={fieldClass} /><DatePicker ariaLabel="Goal target date" dataTestId="finance-goal-target-date" value={goal.targetDate} onChange={(targetDate) => setGoal({ ...goal, targetDate })} placeholder="Target date (optional)" className={fieldClass} /><Input required maxLength={3} aria-label="Goal currency" value={goal.currency} onChange={(event) => setGoal({ ...goal, currency: event.target.value.toUpperCase() })} className={fieldClass} /><Button disabled={mutation.isPending} className="sm:col-span-2"><Plus className="mr-2 h-4 w-4" />Add goal</Button></form><div className="mt-5 space-y-3">{data.goals.filter((item) => item.status !== "archived").map((item) => { const pct = Math.min(100, Math.round(item.currentMinor / item.targetMinor * 100)); return <div key={item.id} className="rounded-lg border border-muted/20 p-3" data-testid={`finance-goal-${item.id}`}><div className="flex justify-between"><span>{item.name}</span><span className="font-mono text-xs text-primary">{item.status === "completed" ? "complete" : `${pct}%`}</span></div><p className="mt-1 text-xs text-muted-foreground">{money(item.currentMinor, item.currency)} of {money(item.targetMinor, item.currency)}{item.targetDate ? ` · ${item.targetDate}` : ""}</p><div className="mt-3 flex flex-wrap gap-2">{item.status === "active" && <><Input inputMode="decimal" aria-label={`Progress for ${item.name}`} placeholder="Current progress" value={goalDrafts[item.id] ?? ""} onChange={(event) => setGoalDrafts({ ...goalDrafts, [item.id]: event.target.value })} className={`${fieldClass} h-9 min-w-28 flex-1`} /><Button type="button" variant="outline" size="sm" aria-label={`Update progress for ${item.name}`} onClick={() => updateGoal(item, { currentMinor: toMinor(goalDrafts[item.id] ?? String(item.currentMinor / 100)) })}>Update</Button><Button type="button" variant="ghost" size="sm" aria-label={`Complete ${item.name}`} onClick={() => updateGoal(item, { currentMinor: item.targetMinor, status: "completed" })}><Check className="mr-2 h-4 w-4" />Complete</Button></>}<Button type="button" variant="ghost" size="sm" aria-label={`${item.status === "completed" ? "Reopen" : "Archive"} ${item.name}`} onClick={() => updateGoal(item, { status: item.status === "completed" ? "active" : "archived" })}>{item.status === "completed" ? <><RotateCcw className="mr-2 h-4 w-4" />Reopen</> : <><Archive className="mr-2 h-4 w-4" />Archive</>}</Button></div></div>; })}</div></div>
        </section>
        <p className="mt-6 rounded-lg border border-primary/20 bg-primary/5 p-4 text-xs text-muted-foreground" data-testid="finance-disclosure">{data.disclosure} Bank connection is not yet active; manual tracking remains fully usable.</p>
      </>}
    </div>
  );
}
