...
import { ArrowLeft, Landmark, Loader2, Plus, RefreshCcw, Target, WalletCards } from "lucide-react";
...
type FinanceTransaction = { id: number; accountId: number; amountMinor: number; currency: string; transactionDate: string; description: string; category: string; status: string };
type FinanceBudget = { id: number; month: string; category: string; currency: string; limitMinor: number; spentMinor: number };
...
export default function PersonalFinancePage() {
  ...
  const createTransaction = (event: FormEvent) => {
    ...
    mutation.mutate({ url: "/api/finance/transactions", method: "POST", body: { accountId: Number(transaction.accountId), amountMinor: transaction.direction === "expense" ? -magnitude : magnitude, transactionDate: transaction.transactionDate, description: transaction.description, category: transaction.category, status: "posted", clientMutationId: crypto.randomUUID() } }, { onSuccess: () => setTransaction({ ...transaction, amount: "", description: "", category: "" }) });
  };
