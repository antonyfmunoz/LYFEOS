...
import { ArrowLeft, Clock, Target, Calendar, Layers, Settings, Loader2, TrendingUp, CheckCircle, Timer } from "lucide-react";
import { AreaChart, Area, BarChart, Bar, ResponsiveContainer, XAxis, YAxis, Tooltip, CartesianGrid } from "recharts";

const DAY_OPTIONS = [7, 14, 30, 90];

function getStatusBadge(pct: number): { label: string; color: string; bg: string } {
  if (pct >= 60) return { label: "HIGH", color: "text-primary", bg: "bg-primary/20 border-primary/30" };
...
export default function TimeDetailPage() {
  ...
  const maxTT = stats.timeTokens.max;
  const timePct = maxTT > 0 ? Math.round((currentTT / maxTT) * 100) : 0;

  const missionTimeCost = computedStats?.totalTimeCost ?? 0;
  const allocatedToMissions = Math.round((missionTimeCost / maxTT) * 100);
    ...
    <div className="mx-auto max-w-5xl py-8 px-4">
        ...
        <div className="relative z-10">
          ...
          <div className="mt-6">
            <div className="flex justify-between mb-2">
              <span className="text-xs text-muted-foreground font-mono">Current: {currentTT}/{maxTT} TT</span>
              <span className="text-xs text-muted-foreground font-mono">Capacity: {timePct}%</span>
            </div>
      ...
      {isLoading ? (
        <div className="flex flex-col items-center justify-center py-20 gap-3">
          ...
          <p className="text-sm text-muted-foreground font-mono">Analyzing time data...</p>
        </div>
      ) : (
        <>
