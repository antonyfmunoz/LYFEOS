    ...
    <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
      ...
      {automation ? <main className="space-y-5">
        <section className="space-y-4 rounded-xl border border-primary/15 bg-card/35 p-5">
          ...
          {automation.pauseReason ? <div className="flex items-start gap-2 rounded-lg border border-amber-500/25 bg-amber-500/5 p-3 text-sm"><AlertTriangle className="mt-0.5 h-4 w-4 shrink-0 text-amber-400" /><div><strong>Paused after {automation.consecutiveFailures} consecutive failed runs</strong><p className="text-xs text-muted-foreground">Review and repair the receipts below. Enabling the rule is an explicit reset of the failure counter.</p></div></div> : null}
        </section>
        ...
        <section className="space-y-3 rounded-xl border border-primary/15 bg-card/35 p-5"><div><strong>Run history</strong><p className="text-xs text-muted-foreground">Receipts contain action types, outcomes, attempts, and record IDs—not copied mission descriptions. Repair retries only unfinished actions from the immutable saved rule.</p></div>{detail.data?.runs.length ? <div className="divide-y divide-primary/10">{detail.data.runs.map((receipt) => <div key={receipt.id} className="flex flex-wrap items-center justify-between gap-2 py-3 text-xs"><div><span><strong className="font-medium capitalize">{receipt.status}</strong> · {receipt.triggerType.replaceAll("_", " ")} · {receipt.actionResults.length} action{receipt.actionResults.length === 1 ? "" : "s"}</span><p className="mt-1 text-muted-foreground">{new Date(receipt.createdAt).toLocaleString()}{receipt.actionResults.some((result) => result.attemptCount > 1) ? " · repaired action" : ""}</p></div>{["failed", "partial", "running"].includes(receipt.status) ? <Button size="sm" variant="outline" disabled={repair.isPending && repair.variables === receipt.id} onClick={() => repair.mutate(receipt.id)}><RotateCcw className="mr-1 h-3.5 w-3.5" />Repair</Button> : null}</div>)}</div> : <p className="text-sm text-muted-foreground">No actual runs yet. Previews are intentionally not stored as executions.</p>}</section>
