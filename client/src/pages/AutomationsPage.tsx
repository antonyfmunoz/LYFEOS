...
import { Play, Plus, Save, ShieldCheck, Trash2, Workflow } from "lucide-react";
import {
...
type AutomationRun = {
  ...
  actionResults: Array<{ type: string; status: string; targetQuestId?: number }>;
...
export default function AutomationsPage() {
  ...
  const run = useMutation({
    mutationFn: () => apiRequest<{ result: { status: string } }>(`/api/automations/${selectedId}/run`, { method: "POST", body: JSON.stringify({ questId: missionId }) }),
  ...
  const setTrigger = (type: AutomationTriggerType) => setDefinition((current) => ({ ...current, trigger: { type } }));
  ...
  return <div className="container max-w-6xl space-y-5 py-6">
    ...
    <div className="grid gap-5 lg:grid-cols-[280px_1fr]">
      <aside className="space-y-2 rounded-xl border border-primary/15 bg-card/35 p-3">
        ...
        {list.data?.automations.length ? list.data.automations.map((item) => <button key={item.id} onClick={() => setSelectedId(item.id)} className={`w-full rounded-lg border p-3 text-left transition-colors ${selectedId === item.id ? "border-primary/45 bg-primary/10" : "border-transparent hover:bg-primary/5"}`}><span className="flex items-center justify-between gap-2"><strong className="truncate text-sm font-medium">{item.name}</strong><span className={`h-2 w-2 rounded-full ${item.enabled ? "bg-emerald-400" : "bg-muted-foreground/40"}`} /></span><span className="mt-1 block text-xs text-muted-foreground">{triggerLabels[item.definition.trigger.type]}</span></button>) : <p className="p-5 text-center text-sm text-muted-foreground">No automations yet.</p>}
      </aside>
      ...
      {automation ? <main className="space-y-5">
        ...
        <section className="space-y-4 rounded-xl border border-primary/15 bg-card/35 p-5"><div className="flex items-start gap-2"><ShieldCheck className="mt-0.5 h-5 w-5 text-primary" /><div><strong>Preview and controlled run</strong><p className="text-xs text-muted-foreground">Preview evaluates the last saved rule without writing. “Run now” is available only for an enabled, saved manual rule.</p></div></div><div className="flex flex-wrap gap-2"><select aria-label="Preview mission" className="h-9 min-w-64 flex-1 rounded-md border border-input bg-background px-3 text-sm" value={missionId || ""} onChange={(event) => setMissionId(Number(event.target.value))}><option value="" disabled>Select a mission</option>{missions.data?.missions.map((mission) => <option key={mission.id} value={mission.id}>{mission.completed ? "✓ " : ""}{mission.title}</option>)}</select><Button variant="outline" disabled={!missionId || previewMutation.isPending} onClick={() => previewMutation.mutate()}>Preview</Button><Button disabled={!missionId || automation.definition.trigger.type !== "manual" || !automation.enabled || run.isPending} onClick={() => run.mutate()}><Play className="mr-1 h-4 w-4" />Run now</Button></div>{preview ? <div className={`rounded-lg border p-3 text-sm ${preview.matched ? "border-primary/25 bg-primary/5" : "border-amber-500/25 bg-amber-500/5"}`}><p>{preview.disclosure}</p>{preview.actions.map((action, index) => <p key={index} className="mt-1 text-xs text-muted-foreground">{index + 1}. {action.description}</p>)}</div> : null}</section>

        <section className="space-y-3 rounded-xl border border-primary/15 bg-card/35 p-5"><div><strong>Run history</strong><p className="text-xs text-muted-foreground">Receipts contain action types, outcomes, and record IDs—not copied mission descriptions.</p></div>{detail.data?.runs.length ? <div className="divide-y divide-primary/10">{detail.data.runs.map((receipt) => <div key={receipt.id} className="flex flex-wrap items-center justify-between gap-2 py-3 text-xs"><span><strong className="font-medium capitalize">{receipt.status}</strong> · {receipt.triggerType.replaceAll("_", " ")} · {receipt.actionResults.length} action{receipt.actionResults.length === 1 ? "" : "s"}</span><span className="text-muted-foreground">{new Date(receipt.createdAt).toLocaleString()}</span></div>)}</div> : <p className="text-sm text-muted-foreground">No actual runs yet. Previews are intentionally not stored as executions.</p>}</section>
