...
export default function HealthDataRights({ embedded = false }: { embedded?: boolean }) {
  ...
  return <section className={embedded ? "mb-4 rounded-lg border border-primary/10 bg-background/40 p-4" : "glassmorphic mb-8 rounded-2xl border border-primary/30 p-6"} aria-labelledby="health-rights-heading" data-testid="health-data-rights">
    <div className="flex flex-wrap items-start justify-between gap-3"><div><h2 id="health-rights-heading" className={embedded ? "flex items-center gap-2 text-sm font-medium text-foreground" : "flex items-center gap-2 font-orbitron text-lg text-primary"}><ShieldCheck className={embedded ? "h-4 w-4 text-primary" : "h-5 w-5"} />Health data & permissions</h2><p className={embedded ? "mt-1 text-xs text-muted-foreground" : "mt-1 text-sm text-muted-foreground"}>Your private health domain currently contains {totalRecords} direct records. Export, permission changes, and deletion create a minimal rights receipt.</p></div><Button size="sm" variant="outline" data-testid="health-data-export" onClick={download}><Download />Download health JSON</Button></div>
    <div className={embedded ? "mt-3 space-y-3 rounded-lg border border-primary/10 bg-card/50 p-3" : "mt-4 space-y-3 rounded-xl border border-muted/20 bg-background/20 p-4"}>
    ...
    <div className={embedded ? "mt-3 rounded-lg border border-primary/10 bg-card/50 p-3" : "mt-4 rounded-xl border border-muted/20 bg-background/20 p-4"}><p className="text-sm text-white">Provider connection records</p>{rights.data?.providerConnections.length ? <div className="mt-2 space-y-1">{rights.data.providerConnections.map((connection) => <p key={connection.id} className="text-xs text-muted-foreground">{connection.providerName} · {connection.status} · {connection.scopes.join(", ") || "no scopes"}</p>)}</div> : <p className="mt-1 text-xs text-muted-foreground">No provider consent intent or connection record exists.</p>}<p className="mt-2 text-[11px] text-muted-foreground">Manage consent, pause, revoke, and imported-data deletion in Health connections. Tokens and credential references are never displayed here.</p></div>
    {rights.data?.retentionBehavior ? <div className={embedded ? "mt-3 rounded-lg border border-primary/10 bg-card/50 p-3" : "mt-4 rounded-xl border border-muted/20 bg-background/20 p-4"}><p className="text-sm text-white">Current data-retention behavior</p><p className="mt-2 text-xs text-amber-300">This describes what the code currently does. It is not an approved legal retention policy, and no automatic timed purge is configured.</p><dl className="mt-3 space-y-2 text-xs"><div><dt className="text-white">Native records</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.nativeRecords}</dd></div><div><dt className="text-white">Provider records</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.providerRecords}</dd></div><div><dt className="text-white">Deleted-provider suppression</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.suppressionHashes}</dd></div><div><dt className="text-white">Credential references</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.credentialReferences}</dd></div><div><dt className="text-white">Rights receipts</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.rightsReceipts}</dd></div></dl></div> : null}
    <div className={embedded ? "mt-3 rounded-lg border border-destructive/25 bg-destructive/5 p-3" : "mt-4 rounded-xl border border-destructive/25 bg-destructive/5 p-4"}>
      <p className="text-sm text-destructive">Delete private health-domain data</p>
      ...
      <Input className="mt-3" data-testid="health-data-deletion-confirmation" aria-label="Health data deletion confirmation" placeholder="Type DELETE MY HEALTH DATA" value={confirmation} onChange={(event) => setConfirmation(event.target.value)} />
      <Button className="mt-3" variant="destructive" data-testid="health-data-delete" disabled={confirmation !== "DELETE MY HEALTH DATA" || deleteHealth.isPending} onClick={() => deleteHealth.mutate()}><Trash2 />Delete health data</Button>
      {deleteHealth.isSuccess ? <p className="mt-2 text-xs text-primary" data-testid="health-data-deletion-complete">Health-domain deletion completed.</p> : null}
