...
export default function HealthDataRights({ embedded = false }: { embedded?: boolean }) {
  ...
  return <section className={embedded ? "mb-4 rounded-lg border border-primary/10 bg-background/40 p-4" : "glassmorphic mb-8 rounded-2xl border border-primary/30 p-6"} aria-labelledby="health-rights-heading" data-testid="health-data-rights">
    ...
    {rights.data?.retentionBehavior ? <details className={embedded ? "mt-3 rounded-lg border border-primary/10 bg-card/50 p-3" : "mt-4 rounded-xl border border-muted/20 bg-background/20 p-4"}><summary className="cursor-pointer text-sm text-white">Current data-retention behavior</summary><p className="mt-2 text-xs text-amber-300">This describes what the code currently does. It is not an approved legal retention policy, and no automatic timed purge is configured.</p><dl className="mt-3 space-y-2 text-xs"><div><dt className="text-white">Native records</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.nativeRecords}</dd></div><div><dt className="text-white">Provider records</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.providerRecords}</dd></div><div><dt className="text-white">Deleted-provider suppression</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.suppressionHashes}</dd></div><div><dt className="text-white">Credential references</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.credentialReferences}</dd></div><div><dt className="text-white">Rights receipts</dt><dd className="text-muted-foreground">{rights.data.retentionBehavior.rightsReceipts}</dd></div></dl></details> : null}
    <details className={embedded ? "mt-3 rounded-lg border border-destructive/25 bg-destructive/5 p-3" : "mt-4 rounded-xl border border-destructive/25 bg-destructive/5 p-4"}>
