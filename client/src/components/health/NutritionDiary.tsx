...
type DiaryEntry = { id: number; foodId: number; foodName: string; foodBrand: string | null; servingGrams: number; inputQuantity: number | null; inputUnit: "g" | "serving" | "ml" | "portion" | null; inputPortionId: number | null; inputUnitLabel: string | null; inputGramsPerUnit: number | null; foodEvidenceSnapshot: { source: string; catalogProviderId: string | null; catalogDatasetVersion: string | null; catalogItemVersion: string | null; catalogEvidence: Food["catalogEvidence"]; catalogSourceModified: boolean } | null; mealSlot: string; nutrients: Nutrient[] };
...
function diaryEvidenceLabel(entry: DiaryEntry): string {
  ...
  if (entry.foodEvidenceSnapshot.source !== "catalog") return "manual source record";
  const source = entry.foodEvidenceSnapshot.catalogEvidence?.sourceKind;
  return source === "government_reference_database" ? "government reference snapshot" : source === "government_branded_database" ? "government branded-record snapshot" : source === "community_catalog" ? "community catalog snapshot" : "catalog source snapshot";
}
