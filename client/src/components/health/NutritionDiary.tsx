...
import { Apple, Camera, ChevronDown, ChevronLeft, ChevronRight, Pencil, Plus, Settings2, Star, Trash2, Utensils, X } from "lucide-react";
...
const loadFoodCatalogSearch = () => import("./FoodCatalogSearch");
const loadNutritionReportsPanel = () => import("./NutritionReportsPanel");
const loadRecipeImportDraft = () => import("./RecipeImportDraft");
...
type Food = { id: number; name: string; brand: string | null; barcode: string | null; servingSizeGrams: number; densityGramsPerMl: number | null; favorite: boolean; note: string | null; recentUseCount: number; lastLoggedAt: string | null; catalogProviderId: string | null; catalogDatasetVersion: string | null; catalogItemVersion: string | null; catalogAttributionText: string | null; catalogAttributionUrl: string | null; catalogEvidence: { sourceKind: "community_catalog" | "government_branded_database" | "government_reference_database" | "provider_classification_unavailable"; measurementBasis: "catalog_or_label_reported" | "government_reference" | "provider_basis_unavailable"; recordUpdatedAt: string | null; reportedNutrientCount: number; reportedCoreNutrientKeys: string[] } | null; catalogSourceModified: boolean; nutrients: Nutrient[]; portions: FoodPortion[] };
type DiaryEntry = { id: number; foodId: number; foodName: string; foodBrand: string | null; servingGrams: number; inputQuantity: number | null; inputUnit: "g" | "serving" | "ml" | "portion" | null; inputPortionId: number | null; inputUnitLabel: string | null; inputGramsPerUnit: number | null; foodEvidenceSnapshot: { source: string; catalogProviderId: string | null; catalogDatasetVersion: string | null; catalogItemVersion: string | null; catalogEvidence: Food["catalogEvidence"]; catalogSourceModified: boolean; recordingMethod?: "manual" | "private_photo_reference" } | null; mealSlot: string; nutrients: Nutrient[] };
type Diary = { date: string; entries: DiaryEntry[]; totals: Record<string, { amount: number; unit: string }> };
type Recipe = { id: number; name: string; servings: number; folder: string | null; note: string | null; sourceUrl: string | null; revisionCount: number; currentRevision: number; ingredients: Array<{ foodId: number; grams: number; food: { name: string } | null }> };
type RecipeRevision = { revisionNumber: number; name: string; servings: number; folder: string | null; note: string | null; sourceUrl: string | null; ingredientsSnapshot: Array<{ foodId: number; grams: number; sortOrder: number }>; createdAt: string };
...
function diaryEvidenceLabel(entry: DiaryEntry): string {
  if (!entry.foodEvidenceSnapshot) return "legacy source context unavailable";
  const reviewPrefix = entry.foodEvidenceSnapshot.recordingMethod === "private_photo_reference" ? "private photo reviewed · " : "";
  if (entry.foodEvidenceSnapshot.source !== "catalog") return reviewPrefix + "manual source record";
  const source = entry.foodEvidenceSnapshot.catalogEvidence?.sourceKind;
  return reviewPrefix + (source === "government_reference_database" ? "government reference snapshot" : source === "government_branded_database" ? "government branded-record snapshot" : source === "community_catalog" ? "community catalog snapshot" : "catalog source snapshot");
}
...
export default function NutritionDiary({ importedFoodId, onImportedFoodHandled, manualFoodRequest, onManualFoodHandled }: { importedFoodId?: number | null; onImportedFoodHandled?: () => void; manualFoodRequest?: { name: string } | null; onManualFoodHandled?: () => void }) {
  ...
  const { attempt: recipeImportAttempt, Component: RecipeImportDraft, retry: retryRecipeImport } = useDeferredFeature(loadRecipeImportDraft);
  const { user } = useAuth();
  ...
  const photoReferenceInput = useRef<HTMLInputElement>(null);
  const [photoReferenceUrl, setPhotoReferenceUrl] = useState<string | null>(null);
  const [photoReferenceName, setPhotoReferenceName] = useState("");
  const [photoReferenceReviewed, setPhotoReferenceReviewed] = useState(false);
  ...
  const [recipeImportLines, setRecipeImportLines] = useState<string[]>([]);
  ...
  const setPhotoReference = (file: File | undefined) => {
    if (!file) return;
    if (!file.type.startsWith("image/")) { toast({ title: "Choose an image", description: "The private reference must be a photo or image file.", variant: "destructive" }); return; }
    setPhotoReferenceUrl(URL.createObjectURL(file)); setPhotoReferenceName(file.name); setPhotoReferenceReviewed(false);
  };
  useEffect(() => {
  ...
  const logFood = useMutation({
    ...
    mutationFn: async () => {
      ...
      return submitHealthMutation({ userId: user.id, url: "/api/nutrition/diary", body: { foodId: Number(selectedFoodId), quantity: Number(servingGrams), inputUnit, portionId: inputUnit === "portion" ? Number(inputPortionId) : null, reviewMethod: photoReferenceUrl ? "private_photo_reference" : "manual", mealSlot, occurredAt: localDateTimeIso(date, entryTime) } });
    },
    onSuccess: (result) => { setServingGrams(inputUnit === "serving" ? "1" : selectedFood ? String(selectedFood.servingSizeGrams) : "100"); clearPhotoReference(); setEditingEntryId(null); if (result && typeof result === "object" && "queued" in result && result.queued) { void queryClient.invalidateQueries({ queryKey: ["health-offline-queue", user?.id] }); toast({ title: "Nutrition record saved on this device", description: "LyfeOS will add it to your account when this device is online." }); } else void refresh(); },
    onError: (error: Error) => toast({ title: "Nutrition record was not saved", description: error.message, variant: "destructive" }),
  });
  const createRecipe = useMutation({
    mutationFn: () => { const currentRevision = recipes.data?.recipes.find((recipe) => recipe.id === editingRecipeId)?.currentRevision; if (editingRecipeId && !currentRevision) throw new Error("Reload this recipe before saving changes."); return apiRequest<{ recipe: Recipe }>(editingRecipeId ? `/api/nutrition/recipes/${editingRecipeId}` : "/api/nutrition/recipes", { method: editingRecipeId ? "PATCH" : "POST", headers: editingRecipeId ? { "x-lyfeos-expected-revision": String(currentRevision) } : undefined, body: JSON.stringify({ name: recipeName, servings: Number(recipeServings), folder: recipeFolder || null, note: recipeNote || null, sourceUrl: recipeSourceUrl || null, ingredients: recipeIngredients.map((ingredient) => ({ foodId: Number(ingredient.foodId), grams: Number(ingredient.grams) })) }) }); },
    onSuccess: ({ recipe }) => { setRecipeName(""); setRecipeServings("1"); setRecipeFolder(""); setRecipeNote(""); setRecipeSourceUrl(""); setRecipeIngredients([{ foodId: "", grams: "100" }]); setRecipeImportLines([]); setEditingRecipeId(null); setSelectedRecipeId(String(recipe.id)); void refresh(); },
  ...
  return (
    <section className="glassmorphic rounded-2xl p-6 mb-8 border border-primary/30" aria-labelledby="nutrition-heading">
      ...
      {trends.data || contributions.data ? <DeferredFeatureChunkBoundary key={nutritionReportsAttempt} fallback={<div className="mt-5 rounded-lg border border-destructive/30 bg-background/20 p-3 text-xs" role="alert"><p className="font-medium">Nutrition reports could not load.</p><p className="mt-1 text-muted-foreground">Diary logging is still available. Try the report again or reload the latest LyfeOS version.</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={retryNutritionReports}>Try reports again</Button><Button size="sm" variant="ghost" onClick={() => window.location.reload()}>Reload LyfeOS</Button></div></div>}><Suspense fallback={<div className="mt-5 rounded-lg border border-primary/15 bg-background/20 p-3 text-xs text-muted-foreground" role="status">Loading nutrition reports…</div>}><NutritionReportsPanel trendDays={trendDays} trends={trends.data} contributions={contributions.data} contributionNutrient={contributionNutrient} reportDownloadError={reportDownloadError} onTrendDaysChange={(days) => { setTrendDays(days); setReportDownloadError(false); }} onContributionNutrientChange={setContributionNutrient} onDownload={() => void downloadNutritionReport()} /></Suspense></DeferredFeatureChunkBoundary> : null}
      {expanded && <div className="grid gap-4 lg:grid-cols-2 mt-5 pt-5 border-t border-muted/20">
        <DeferredFeatureChunkBoundary key={foodCatalogAttempt} fallback={<div className="rounded-xl border border-destructive/30 bg-background/25 p-4 text-xs" role="alert"><p className="font-medium">Food catalog could not load.</p><p className="mt-1 text-muted-foreground">Manual foods and the rest of the diary remain available.</p><div className="mt-3 flex flex-wrap gap-2"><Button size="sm" variant="outline" onClick={retryFoodCatalog}>Try catalog again</Button><Button size="sm" variant="ghost" onClick={() => window.location.reload()}>Reload LyfeOS</Button></div></div>}><Suspense fallback={<div className="rounded-xl border border-primary/15 bg-background/25 p-4 text-xs text-muted-foreground">Loading catalog…</div>}><FoodCatalogSearch selectedFood={selectedFood} onImported={(foodId) => { setSelectedFoodId(String(foodId)); void refresh(); }} /></Suspense></DeferredFeatureChunkBoundary>
        <div className="rounded-xl border border-primary/15 bg-background/25 p-4">
          ...
          <div className="grid gap-2 mt-3"><select aria-label="Choose saved food" disabled={editingEntryId !== null} className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={selectedFoodId} onChange={(event) => { setSelectedFoodId(event.target.value); setInputPortionId(""); const food = foods.data?.foods.find((item) => item.id === Number(event.target.value)); if (food) setServingGrams(inputUnit === "serving" ? "1" : String(food.servingSizeGrams)); }}><option value="">Choose a food</option>{foods.data?.foods.map((food) => <option key={food.id} value={food.id}>{food.favorite ? "★ " : ""}{food.name}{food.brand ? ` — ${food.brand}` : ""}{food.recentUseCount ? ` · used ${food.recentUseCount}×` : ""}</option>)}</select><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><Input aria-label="Food quantity" type="number" min="0.01" step="0.01" value={servingGrams} onChange={(event) => setServingGrams(event.target.value)} /><select aria-label="Food quantity unit" className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={inputUnit} onChange={(event) => { const nextUnit = event.target.value as "g" | "serving" | "ml" | "portion"; setInputUnit(nextUnit); setInputPortionId(nextUnit === "portion" ? String(selectedFood?.portions[0]?.id || "") : ""); setServingGrams(nextUnit === "g" ? selectedFood ? String(selectedFood.servingSizeGrams) : "100" : "1"); }}><option value="g">grams</option><option value="serving">servings</option>{selectedFood?.densityGramsPerMl ? <option value="ml">milliliters</option> : null}{selectedFood?.portions.length ? <option value="portion">saved portion</option> : null}</select><select aria-label="Meal" className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={mealSlot} onChange={(event) => setMealSlot(event.target.value)}>{["breakfast", "lunch", "dinner", "snack", "other"].map((slot) => <option key={slot} value={slot}>{slot}</option>)}</select><Input aria-label="Meal time" type="time" disabled={editingEntryId !== null} value={entryTime} onChange={(event) => setEntryTime(event.target.value)} /></div>{editingEntryId ? <p className="text-[11px] text-muted-foreground">This correction keeps the entry’s original recorded time.</p> : null}{inputUnit === "portion" ? <select aria-label="Saved food portion" className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={inputPortionId} onChange={(event) => setInputPortionId(event.target.value)}>{selectedFood?.portions.map((portion) => <option key={portion.id} value={portion.id}>{portion.label} · {portion.gramsPerUnit} g each</option>)}</select> : null}{selectedFood && inputUnit === "serving" ? <p className="text-xs text-muted-foreground">1 serving = {selectedFood.servingSizeGrams} g for this food.</p> : null}{selectedFood && inputUnit === "ml" ? <p className="text-xs text-muted-foreground">1 ml = {selectedFood.densityGramsPerMl} g using this food’s saved density.</p> : null}
            {!editingEntryId ? <div className="rounded-lg border border-primary/15 bg-background/20 p-3" aria-label="Private meal photo reference"><input ref={photoReferenceInput} className="sr-only" type="file" accept="image/*" capture="environment" aria-label="Choose a private meal reference photo" onChange={(event) => setPhotoReference(event.target.files?.[0])} /><div className="flex flex-wrap items-start justify-between gap-2"><div><p className="flex items-center gap-2 text-xs font-medium"><Camera className="h-3.5 w-3.5 text-primary" />Private photo reference</p><p className="mt-1 text-[11px] text-muted-foreground">Use a photo to help you identify the exact food and serving. It stays in this browser tab, is never uploaded or stored, and LyfeOS makes no food or portion guess from it.</p></div>{photoReferenceUrl ? <Button type="button" size="sm" variant="ghost" onClick={clearPhotoReference}><X />Remove photo</Button> : <Button type="button" size="sm" variant="outline" onClick={() => photoReferenceInput.current?.click()}><Camera />Use photo</Button>}</div>{photoReferenceUrl ? <div className="mt-3 flex flex-wrap items-start gap-3"><img src={photoReferenceUrl} alt="Private meal reference preview; not uploaded" className="h-20 w-20 rounded-md border border-muted/20 object-cover" /><div className="min-w-0 flex-1"><p className="truncate text-xs text-muted-foreground">{photoReferenceName}</p><label className="mt-2 flex items-start gap-2 text-xs text-muted-foreground"><input aria-label="Confirm photo-reviewed food and quantity" type="checkbox" checked={photoReferenceReviewed} onChange={(event) => setPhotoReferenceReviewed(event.target.checked)} /><span>I confirmed the exact saved food and quantity from this reference. Only that review method—not the photo—is recorded with the diary entry.</span></label></div></div> : null}</div> : null}
            <div className="flex flex-wrap gap-2"><Button disabled={!selectedFoodId || invalidNumber(servingGrams) || (!editingEntryId && !entryTime) || (inputUnit === "portion" && !inputPortionId) || Boolean(photoReferenceUrl && !photoReferenceReviewed) || logFood.isPending} onClick={() => logFood.mutate()}>{editingEntryId ? <Pencil /> : <Plus />}{editingEntryId ? "Update entry" : "Add to diary"}</Button>{editingEntryId ? <Button variant="ghost" onClick={() => setEditingEntryId(null)}>Cancel</Button> : <Button variant="ghost" disabled={copyPreviousMeal.isPending} onClick={() => copyPreviousMeal.mutate()}>Copy previous {mealSlot}</Button>}</div>{copyPreviousMeal.error ? <p className="text-xs text-destructive">Could not copy that meal. The source must have entries and the target meal must be empty.</p> : null}</div>
        ...
        <div className="rounded-xl border border-primary/15 bg-background/25 p-4 lg:col-span-2">
          ...
          <DeferredFeatureChunkBoundary key={recipeImportAttempt} fallback={<div className="mt-3 rounded-lg border border-destructive/30 bg-background/20 p-3 text-xs" role="alert"><p className="font-medium">Recipe URL import could not load.</p><p className="mt-1 text-muted-foreground">Create the meal manually, or try importing again.</p><Button className="mt-3" size="sm" variant="outline" onClick={retryRecipeImport}>Try recipe import again</Button></div>}><Suspense fallback={<div className="mt-3 rounded-lg border border-primary/15 bg-background/20 p-3 text-xs text-muted-foreground">Loading recipe import…</div>}><RecipeImportDraft onDraftReady={(draft) => { setRecipeName(draft.name); setRecipeSourceUrl(draft.sourceUrl); setRecipeIngredients(draft.ingredients.map(() => ({ foodId: "", grams: "" }))); setRecipeImportLines(draft.ingredients); }} /></Suspense></DeferredFeatureChunkBoundary>
          <div className="grid gap-2 mt-3 sm:grid-cols-[1fr_8rem_10rem]"><Input aria-label="Recipe name" placeholder="Meal name" value={recipeName} onChange={(event) => setRecipeName(event.target.value)} /><Input aria-label="Recipe yield servings" type="number" min="0.01" step="0.01" placeholder="Servings" value={recipeServings} onChange={(event) => setRecipeServings(event.target.value)} /><Input aria-label="Recipe folder" placeholder="Folder" value={recipeFolder} onChange={(event) => setRecipeFolder(event.target.value)} /></div><Input aria-label="Recipe source URL" className="mt-2" type="url" placeholder="Source URL" value={recipeSourceUrl} onChange={(event) => setRecipeSourceUrl(event.target.value)} /><Input aria-label="Recipe note" className="mt-2" maxLength={500} placeholder="Note" value={recipeNote} onChange={(event) => setRecipeNote(event.target.value)} />
          <div className="space-y-2 mt-2">{recipeIngredients.map((ingredient, index) => <div className="space-y-1" key={index}>{recipeImportLines[index] ? <p className="text-[11px] text-muted-foreground">Publisher line {index + 1}: {recipeImportLines[index]}</p> : null}<div className="grid grid-cols-[1fr_7rem_auto] gap-2"><select aria-label={`Recipe ingredient ${index + 1}`} className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={ingredient.foodId} onChange={(event) => setRecipeIngredients((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, foodId: event.target.value } : item))}><option value="">Choose a food</option>{foods.data?.foods.map((food) => <option key={food.id} value={food.id}>{food.name}</option>)}</select><Input aria-label={`Ingredient ${index + 1} grams`} type="number" min="1" placeholder="Measured grams" value={ingredient.grams} onChange={(event) => setRecipeIngredients((items) => items.map((item, itemIndex) => itemIndex === index ? { ...item, grams: event.target.value } : item))} /><Button variant="ghost" size="sm" aria-label={`Remove ingredient ${index + 1}`} disabled={recipeIngredients.length === 1} onClick={() => { setRecipeIngredients((items) => items.filter((_, itemIndex) => itemIndex !== index)); setRecipeImportLines((lines) => lines.filter((_, lineIndex) => lineIndex !== index)); }}>Remove</Button></div></div>)}</div>
          <div className="flex flex-wrap gap-2 mt-2"><Button variant="outline" size="sm" onClick={() => { setRecipeIngredients((items) => [...items, { foodId: "", grams: "100" }]); setRecipeImportLines((lines) => [...lines, ""]); }}><Plus />Ingredient</Button><Button variant="outline" size="sm" disabled={!recipeName.trim() || invalidNumber(recipeServings) || recipeIngredients.some((ingredient) => !ingredient.foodId || invalidNumber(ingredient.grams)) || new Set(recipeIngredients.map((ingredient) => ingredient.foodId)).size !== recipeIngredients.length || createRecipe.isPending} onClick={() => createRecipe.mutate()}>{editingRecipeId ? <Pencil /> : <Plus />}{editingRecipeId ? "Update meal" : "Save meal"}</Button>{editingRecipeId ? <Button variant="ghost" size="sm" onClick={() => { setEditingRecipeId(null); setRecipeName(""); setRecipeServings("1"); setRecipeFolder(""); setRecipeNote(""); setRecipeSourceUrl(""); setRecipeIngredients([{ foodId: "", grams: "100" }]); setRecipeImportLines([]); }}>Cancel</Button> : null}</div>
          {recipes.data?.recipes.length ? <>
            <div className="flex flex-col gap-2 mt-3 sm:flex-row">
              ...
              <Button variant="ghost" size="sm" disabled={!selectedRecipeId} onClick={() => { const recipe = recipes.data?.recipes.find((item) => item.id === Number(selectedRecipeId)); if (!recipe) return; setEditingRecipeId(recipe.id); setRecipeName(recipe.name); setRecipeServings(String(recipe.servings)); setRecipeFolder(recipe.folder || ""); setRecipeNote(recipe.note || ""); setRecipeSourceUrl(recipe.sourceUrl || ""); setRecipeIngredients(recipe.ingredients.map((ingredient) => ({ foodId: String(ingredient.foodId), grams: String(ingredient.grams) }))); setRecipeImportLines([]); }}><Pencil />Edit</Button>
            ...
            {selectedRecipe?.sourceUrl ? <a className="mt-2 block break-all text-xs text-primary underline" href={selectedRecipe.sourceUrl} target="_blank" rel="noreferrer">Source</a> : null}{selectedRecipe?.note ? <p className="mt-1 text-[11px] text-muted-foreground">Note: {selectedRecipe.note}</p> : null}
