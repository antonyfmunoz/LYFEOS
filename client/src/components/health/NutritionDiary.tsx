...
type RecipeImportPreview = { draft: { sourceUrl: string; name: string; yieldText: string | null; ingredients: string[]; extractionMethod: "structured_recipe_json_ld" }; disclosure: string };
...
export default function NutritionDiary({ importedFoodId, onImportedFoodHandled, manualFoodRequest, onManualFoodHandled }: { importedFoodId?: number | null; onImportedFoodHandled?: () => void; manualFoodRequest?: { name: string } | null; onManualFoodHandled?: () => void }) {
  ...
  const [recipeImportDisclosure, setRecipeImportDisclosure] = useState("");
  ...
  const createRecipe = useMutation({
    ...
    onSuccess: ({ recipe }) => { setRecipeName(""); setRecipeServings("1"); setRecipeFolder(""); setRecipeIngredients([{ foodId: "", grams: "100" }]); setRecipeImportLines([]); setRecipeImportDisclosure(""); setEditingRecipeId(null); setSelectedRecipeId(String(recipe.id)); void refresh(); },
  ...
  const importRecipePreview = useMutation({
    mutationFn: () => apiRequest<RecipeImportPreview>("/api/nutrition/recipes/import-preview", { method: "POST", body: JSON.stringify({ url: recipeImportUrl }) }),
    onSuccess: ({ draft, disclosure }) => {
      setRecipeName(draft.name);
      setRecipeIngredients(draft.ingredients.map(() => ({ foodId: "", grams: "" })));
      setRecipeImportLines(draft.ingredients);
      setRecipeImportDisclosure(`${disclosure} Source: ${draft.sourceUrl}${draft.yieldText ? ` · Publisher yield: ${draft.yieldText}` : ""}`);
    ...
    <section className="glassmorphic rounded-2xl p-6 mb-8 border border-primary/30" aria-labelledby="nutrition-heading">
      ...
      {expanded && <div className="grid gap-4 lg:grid-cols-2 mt-5 pt-5 border-t border-muted/20">
        ...
        <div className="rounded-xl border border-primary/15 bg-background/25 p-4 lg:col-span-2">
          ...
          <details className="mt-3 rounded-lg border border-primary/15 bg-background/20 p-3">
            ...
            <div className="mt-3 flex flex-col gap-2 sm:flex-row"><Input aria-label="Recipe webpage URL" type="url" inputMode="url" placeholder="https://example.com/recipe" value={recipeImportUrl} onChange={(event) => setRecipeImportUrl(event.target.value)} /><Button variant="outline" disabled={!recipeImportUrl.trim() || importRecipePreview.isPending} onClick={() => importRecipePreview.mutate()}>{importRecipePreview.isPending ? "Reading recipe…" : "Use recipe draft"}</Button></div>
            {recipeImportDisclosure ? <p className="mt-2 break-words text-[11px] text-muted-foreground">{recipeImportDisclosure}</p> : null}
          </details>
