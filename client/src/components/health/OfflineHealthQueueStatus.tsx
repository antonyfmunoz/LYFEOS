...
export default function OfflineHealthQueueStatus() {
  ...
  if (queue.isError) return <section className="mb-8 rounded-xl border border-amber-400/30 bg-amber-400/10 p-4" role="status" data-testid="health-offline-storage-unavailable"><p className="flex items-center gap-2 text-sm text-amber-100"><Database className="h-4 w-4" />Private offline health storage is unavailable.</p><p className="mt-1 text-xs text-amber-100/80">Stay online while saving records on this browser. A failed save remains in its open form; LyfeOS does not claim it was queued.</p></section>;
