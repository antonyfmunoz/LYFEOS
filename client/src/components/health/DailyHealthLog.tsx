...
export default function DailyHealthLog() {
  ...
  return (
    <section className="glassmorphic rounded-2xl p-6 mb-8 border border-primary/30" aria-labelledby="daily-health-heading" data-testid="daily-health-log">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        <div className="rounded-xl border border-primary/20 bg-background/30 p-4">
          ...
          <div className="flex gap-2"><Input aria-label="Hydration amount" data-testid="health-hydration-amount" type="number" min="0.01" step="any" max="20000" value={hydrationMl} onChange={(event) => setHydrationMl(event.target.value)} /><select aria-label="Hydration unit" data-testid="health-hydration-unit" className="h-10 rounded-md border border-input bg-background px-2 text-sm" value={hydrationUnit} onChange={(event) => setHydrationUnit(event.target.value as "ml" | "l" | "fl_oz" | "cup")}><option value="ml">ml</option><option value="l">L</option><option value="fl_oz">fl oz</option><option value="cup">cup</option></select><Button size="sm" data-testid="health-hydration-save" disabled={!Number(hydrationMl) || addHydration.isPending} onClick={() => addHydration.mutate()}>{editingHydrationId ? <Pencil /> : <Plus />}{editingHydrationId ? "Update" : "Log"}</Button>{editingHydrationId ? <Button variant="ghost" size="sm" onClick={() => { setEditingHydrationId(null); setHydrationMl("250"); setHydrationUnit("ml"); }}>Cancel</Button> : null}</div>
      ...
      <div className="rounded-xl border border-primary/20 bg-background/30 p-4 mt-4">
        ...
        <div className="grid gap-2 sm:grid-cols-[1fr_7rem_6rem_auto_auto]"><Input aria-label="Supplement name" data-testid="health-supplement-name" placeholder="Name" value={supplementName} onChange={(event) => setSupplementName(event.target.value)} /><Input aria-label="Supplement amount" data-testid="health-supplement-amount" type="number" min="0" step="0.01" placeholder="Amount" value={supplementAmount} onChange={(event) => setSupplementAmount(event.target.value)} /><Input aria-label="Supplement unit" data-testid="health-supplement-unit" placeholder="Unit" value={supplementUnit} onChange={(event) => setSupplementUnit(event.target.value)} /><Button size="sm" data-testid="health-supplement-save" disabled={!supplementName.trim() || (supplementAmount !== "" && (!Number(supplementAmount) || !supplementUnit.trim())) || addSupplement.isPending} onClick={() => addSupplement.mutate()}>{editingSupplementId ? <Pencil /> : <Plus />}{editingSupplementId ? "Update" : "Log"}</Button>{editingSupplementId ? <Button variant="ghost" size="sm" onClick={() => { setEditingSupplementId(null); setSupplementName(""); setSupplementAmount(""); }}>Cancel</Button> : null}</div>
