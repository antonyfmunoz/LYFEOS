...
type CsvPreview = { importHash: string; validCount: number; invalidCount: number; disclosure: string; rows: Array<{ rowNumber: number; entry: { displayName: string; value: number; unit: string; observedAt: string; sourceRecordId: string; source: "lab" | "imported" } | null; errors: string[] }> };
...
export default function HealthMetricsLedger() {
  ...
  const [csvDefaultCategory, setCsvDefaultCategory] = useState("lab");
  ...
  const previewCsv = useMutation({ mutationFn: () => apiRequest<CsvPreview>("/api/health-observations/imports/preview", { method: "POST", body: JSON.stringify({ csvText, defaultCategory: csvDefaultCategory || undefined }) }), onSuccess: setCsvPreview });
  const commitCsv = useMutation({ mutationFn: () => apiRequest<{ importedCount: number }>("/api/health-observations/imports/commit", { method: "POST", body: JSON.stringify({ csvText, defaultCategory: csvDefaultCategory || undefined, importHash: csvPreview?.importHash, confirmed: true }) }), onSuccess: (result) => { setCsvText(""); setCsvPreview(null); void queryClient.invalidateQueries({ queryKey: ["/api/health-observations"] }); toast({ title: `${result.importedCount} health records imported`, description: "Each row remains a private transcribed record with its source ID." }); } });
  const chooseCsvFile = async (file: File | undefined) => {
    setCsvFileError(null); setCsvPreview(null);
    if (!file) return;
    if (!file.name.toLowerCase().endsWith(".csv") || file.size > 250_000) { setCsvFileError("Choose a .csv file no larger than 250 KB."); return; }
    try { setCsvText(await file.text()); } catch { setCsvFileError("Could not read that CSV file."); }
  };
  const downloadCsvTemplate = () => {
    const csv = "source_record_id,metric_key,display_name,category,value,unit,observed_at,lab_name,method,method_version,device_name,specimen_type,collected_at,reference_low,reference_high,reference_unit,note\r\nlab-001,ldl,LDL cholesterol,lab,120,mg/dL,2026-09-01T10:00:00Z,Example Lab,,,,blood,2026-09-01T09:30:00Z,0,129,mg/dL,Copy values exactly as reported\r\n";
    const url = URL.createObjectURL(new Blob([csv], { type: "text/csv;charset=utf-8" })); const anchor = document.createElement("a"); anchor.href = url; anchor.download = "lyfeos-health-import-template.csv"; anchor.click(); URL.revokeObjectURL(url);
  };
  ...
  return <section className="glassmorphic rounded-2xl p-6 mb-8 border border-primary/30" aria-labelledby="metrics-ledger-heading" data-testid="health-metrics-ledger">
    ...
    <details className="mt-4 rounded-xl border border-primary/15 bg-background/20 p-4"><summary className="cursor-pointer text-sm font-medium text-white flex items-center gap-2"><Upload className="h-4 w-4 text-primary" />Import a reviewed health CSV</summary><p className="mt-2 text-xs text-muted-foreground">Select a CSV or paste up to 200 rows, preview every result, then explicitly import. Required: <span className="font-mono">source_record_id, value, unit, observed_at</span>. A timezone-bearing ISO timestamp and a stable source ID are always required. Common lab headings such as <span className="font-mono">Lab Result ID, Accession Number, Test Name, Result, Units, Collection Date</span> are recognized; Test Name can provide both the visible name and normalized metric key.</p><div className="mt-3 flex flex-wrap items-center gap-2"><label className="inline-flex h-9 cursor-pointer items-center rounded-md border border-input px-3 text-xs font-medium hover:bg-accent"><Upload className="mr-2 h-3.5 w-3.5" />Choose CSV<Input aria-label="Choose health CSV file" className="sr-only" type="file" accept=".csv,text/csv" onChange={(event) => { void chooseCsvFile(event.target.files?.[0]); event.currentTarget.value = ""; }} /></label><Button size="sm" variant="ghost" onClick={downloadCsvTemplate}>Download template</Button><label className="flex items-center gap-2 text-xs text-muted-foreground">If Category is absent<select aria-label="CSV fallback category" className="h-9 rounded-md border border-input bg-background px-2 text-xs" value={csvDefaultCategory} onChange={(event) => { setCsvDefaultCategory(event.target.value); setCsvPreview(null); }}><option value="">Require it in CSV</option>{categories.map((item) => <option key={item} value={item}>{title(item)}</option>)}</select></label></div><textarea aria-label="Health CSV import" className="mt-3 min-h-32 w-full rounded-md border border-input bg-background p-3 font-mono text-xs text-foreground" placeholder="source_record_id,metric_key,display_name,category,value,unit,observed_at\nlab-001,ldl,LDL,lab,120,mg/dL,2026-09-01T10:00:00Z" value={csvText} onChange={(event) => { setCsvText(event.target.value); setCsvPreview(null); setCsvFileError(null); }} /><div className="mt-2 flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={!csvText.trim() || previewCsv.isPending} onClick={() => previewCsv.mutate()}>{previewCsv.isPending ? "Checking…" : "Preview CSV"}</Button>{csvPreview ? <Button size="sm" disabled={csvPreview.invalidCount > 0 || !csvPreview.validCount || commitCsv.isPending} onClick={() => commitCsv.mutate()}>{commitCsv.isPending ? "Importing…" : `Import ${csvPreview.validCount} reviewed rows`}</Button> : null}</div>{csvFileError ? <p className="mt-2 text-xs text-destructive">{csvFileError}</p> : null}{previewCsv.error ? <p className="mt-2 text-xs text-destructive">Could not preview this CSV. Check the stable source IDs, values, and timezone-bearing ISO timestamps.</p> : null}{csvPreview ? <div className="mt-3 rounded-md border border-muted/20 p-3 text-xs"><p className="font-medium">Preview: {csvPreview.validCount} valid · {csvPreview.invalidCount} blocked</p><p className="mt-1 text-muted-foreground">{csvPreview.disclosure}</p><div className="mt-2 space-y-1">{csvPreview.rows.slice(0, 20).map((row) => <p key={row.rowNumber} className={row.errors.length ? "text-destructive" : "text-muted-foreground"}>Row {row.rowNumber}: {row.errors.length ? row.errors.join(" ") : `${row.entry?.displayName} · ${row.entry?.value} ${row.entry?.unit} · ${row.entry?.source === "lab" ? "lab record" : "imported record"} · ${row.entry?.sourceRecordId}`}</p>)}</div>{csvPreview.rows.length > 20 ? <p className="mt-2 text-muted-foreground">Only the first 20 preview rows are displayed; the same validated CSV is imported atomically.</p> : null}</div> : null}{commitCsv.error ? <p className="mt-2 text-xs text-destructive">Could not import this CSV. It may have changed, the selected fallback category may have changed, or it may include source records already imported.</p> : null}</details>
    ...
    <div className="grid gap-2 mt-4 sm:grid-cols-4">
      ...
      <Input aria-label="Metric name" data-testid="health-observation-name" disabled={definitionLocked} placeholder="Metric name" value={displayName} onChange={(event) => { setDisplayName(event.target.value); setMetricKey(event.target.value); }} />
      <Input aria-label="Metric unit" data-testid="health-observation-unit" disabled={definitionLocked} placeholder="Unit" value={unit} onChange={(event) => setUnit(event.target.value)} />
    </div>
    <div className="grid gap-2 mt-2 sm:grid-cols-4"><Input aria-label="Metric value" data-testid="health-observation-value" type="number" step="any" placeholder="Value" value={value} onChange={(event) => setValue(event.target.value)} /><Input aria-label="Observed date" type="date" disabled={!!editingId} value={observedDate} onChange={(event) => setObservedDate(event.target.value)} /><Input aria-label="Measurement method" placeholder="Method (optional)" value={method} onChange={(event) => setMethod(event.target.value)} /><Input aria-label="Measurement method version" placeholder="Method version (optional)" value={methodVersion} onChange={(event) => setMethodVersion(event.target.value)} /></div>
    <div className="grid gap-2 mt-2 sm:grid-cols-4"><select aria-label="Metric source" className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={source} onChange={(event) => setSource(event.target.value)}><option value="manual">Manual</option><option value="device">Device (transcribed)</option><option value="imported">Imported (transcribed)</option><option value="lab">Lab</option></select><Input aria-label="Source record identifier" disabled={source === "manual"} placeholder={source === "imported" ? "Source record ID (required)" : "Source record ID (optional)"} value={sourceRecordId} onChange={(event) => setSourceRecordId(event.target.value)} /><Input aria-label="Device name" placeholder={source === "device" ? "Device name (required)" : "Device name (optional)"} value={deviceName} onChange={(event) => setDeviceName(event.target.value)} /><Input aria-label="Lab name or source" placeholder="Lab/source (optional)" value={labName} onChange={(event) => setLabName(event.target.value)} /></div>
    {source === "device" || source === "imported" ? <p className="mt-2 text-[11px] text-muted-foreground">This preserves a transcribed source record. It does not create a live device or provider connection; use Health connections for an authorized native import.</p> : null}
    ...
    <div className="mt-2 flex gap-2"><Button size="sm" data-testid="health-observation-save" disabled={!keyFromName(metricKey || displayName) || !displayName.trim() || value === "" || !Number.isFinite(Number(value)) || !unit.trim() || save.isPending || (category === "lab" && source !== "lab" && !labName.trim()) || (source === "device" && !deviceName.trim()) || (source === "imported" && !sourceRecordId.trim())} onClick={() => save.mutate()}>{editingId ? <Pencil /> : <Plus />}{editingId ? "Update measurement" : "Save measurement"}</Button>{editingId ? <Button size="sm" variant="ghost" onClick={resetObservation}>Cancel</Button> : null}</div>
