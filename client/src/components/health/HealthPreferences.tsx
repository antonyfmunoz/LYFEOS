...
export default function HealthPreferences({ embedded = false }: { embedded?: boolean }) {
  ...
  return <details className={embedded ? "mb-4 rounded-lg border border-primary/10 bg-background/40 p-4" : "glassmorphic mb-8 rounded-2xl border border-primary/30 p-5"}>
    <summary className={embedded ? "flex cursor-pointer items-center gap-2 text-sm font-medium text-foreground" : "flex cursor-pointer items-center gap-2 font-orbitron text-sm text-primary"}><Settings2 className="h-4 w-4 text-primary" />Health units & calendar context</summary>
