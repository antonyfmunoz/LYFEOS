...
export default function HealthPreferences({ embedded = false, section = "all" }: { embedded?: boolean; section?: "all" | "preferences" | "calendar" }) {
  ...
  const showPreferences = section !== "calendar";
  const showCalendar = section !== "preferences";
  const title = section === "calendar" ? "Calendar context" : section === "preferences" ? "Health display & reminders" : "Health preferences";

  return <section className={embedded ? "mb-4 rounded-lg border border-primary/10 bg-background/40 p-4" : "glassmorphic mb-8 rounded-2xl border border-primary/30 p-5"} aria-label={title}>
    <div className={embedded ? "flex items-center gap-2 text-sm font-medium text-foreground" : "flex items-center gap-2 font-orbitron text-sm text-primary"}><Settings2 className="h-4 w-4 text-primary" />{title}</div>
    {showPreferences ? <>
    <p className="mt-2 text-xs text-muted-foreground">Choose the units, in-app logging cue, and Health workspace shortcuts that suit you. These controls never connect an external provider.</p>
    ...
    <fieldset className={embedded ? "mt-3 rounded-lg border border-primary/10 bg-card/50 p-3" : "mt-3 rounded-lg border border-muted/20 p-3"}><legend className="px-1 text-xs font-medium text-white">Health workspace shortcuts</legend><p className="mb-2 text-[11px] text-muted-foreground">Choose the areas you want quick access to. Everything is optional, and unselected workspaces remain available below.</p><div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-5">{healthTrackingDomains.map((domain) => <label key={domain} className="flex items-center gap-2 text-xs capitalize text-muted-foreground"><input type="checkbox" checked={trackedDomains.includes(domain)} onChange={(event) => setTrackedDomains((current) => event.target.checked ? [...current, domain] : current.filter((item) => item !== domain))} />{domain}</label>)}</div></fieldset>
    <Button className="mt-3" size="sm" disabled={save.isPending} onClick={() => save.mutate()}>Save Health preferences</Button>
    </> : null}
    {showCalendar ? <>
    <p className="mt-2 text-xs text-muted-foreground">New timestamped records retain this device’s IANA timezone and event-time UTC offset. Historical records are never silently rewritten after travel or daylight-saving changes.</p>
    <div className="mt-3 flex flex-col gap-2 sm:flex-row"><div className="relative flex-1"><Clock3 className="pointer-events-none absolute left-3 top-3 h-4 w-4 text-muted-foreground" /><Input className="pl-9" aria-label="Health calendar IANA time zone" value={timeZone} onChange={(event) => setTimeZone(event.target.value)} /></div><Button size="sm" variant="outline" onClick={() => setTimeZone(getBrowserTimeZone())}>Use device timezone</Button><Button size="sm" disabled={!timeZone.trim() || save.isPending} onClick={() => save.mutate()}>Save calendar context</Button></div>
    </> : null}
    {save.isSuccess ? <p className="mt-2 text-xs text-primary" role="status">Health preferences saved.</p> : null}
    {save.error ? <p className="mt-2 text-xs text-destructive" role="alert">Could not save these preferences. Check the IANA timezone name.</p> : null}
  </section>;
}
