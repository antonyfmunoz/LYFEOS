...
export default function SleepLog() {
  ...
  return (
    <section className="glassmorphic rounded-2xl p-6 mb-8 border border-primary/30" aria-labelledby="sleep-log-heading" data-testid="sleep-log">
      ...
      <div className="mt-5 rounded-xl border border-primary/15 bg-background/20 p-3">
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2"><label className="text-xs text-muted-foreground">Started<Input className="mt-1" aria-label="Sleep session started" data-testid="sleep-session-start" type="datetime-local" value={sessionStart} onChange={(event) => setSessionStart(event.target.value)} /></label><label className="text-xs text-muted-foreground">Ended<Input className="mt-1" aria-label="Sleep session ended" data-testid="sleep-session-end" type="datetime-local" value={sessionEnd} onChange={(event) => setSessionEnd(event.target.value)} /></label></div>
        ...
        <div className="mt-2 grid gap-2 sm:grid-cols-[10rem_1fr_auto]"><select aria-label="Subjective session quality" className="h-10 rounded-md border border-input bg-background px-3 text-sm" value={sessionQuality} onChange={(event) => setSessionQuality(event.target.value)}><option value="">Reflection</option>{[1, 2, 3, 4, 5].map((item) => <option key={item} value={item}>{item}/5</option>)}</select><Input aria-label="Sleep session note" maxLength={1000} placeholder="Optional reflection" value={sessionNote} onChange={(event) => setSessionNote(event.target.value)} /><Button size="sm" data-testid="sleep-session-save" disabled={!sessionStart || !sessionEnd || (sessionSource === "transcribed_device" && !sessionDevice.trim()) || saveSession.isPending} onClick={() => saveSession.mutate()}>{editingSessionId ? <Pencil /> : <Plus />}{editingSessionId ? "Correct session" : "Add session"}</Button></div>
