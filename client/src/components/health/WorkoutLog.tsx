...
import { plateBreakdown } from "@/lib/plateCalculator";
...
export default function WorkoutLog() {
  ...
  const [plateUnit, setPlateUnit] = useState<LoadUnit>(preferredLoadUnit);
  const [plateBarWeight, setPlateBarWeight] = useState("20");
  ...
  const plates = plateTarget ? plateBreakdown(Number(plateTarget), plateUnit, Number(plateBarWeight)) : null;
  const observedRecordFor = (exerciseName: string, unit: LoadUnit) => records.data?.records.find((record) => record.exerciseName.localeCompare(exerciseName, undefined, { sensitivity: "accent" }) === 0 && record.loadUnit === unit) || null;
  ...
  return <section className="glassmorphic rounded-2xl p-6 mb-8 border border-primary/30" aria-labelledby="training-heading" data-testid="workout-log">
    ...
    <details className="mt-2 rounded-lg border border-muted/20 bg-background/20 px-3 py-2" aria-label="Barbell plate calculator"><summary className="cursor-pointer text-xs font-medium text-muted-foreground">Plate calculator</summary><div className="mt-2 grid gap-2 sm:grid-cols-[8rem_6rem_8rem]"><Input aria-label="Barbell target load" type="number" min="0.5" step="0.5" placeholder="Total load" value={plateTarget} onChange={(event) => setPlateTarget(event.target.value)} /><select aria-label="Barbell plate calculator unit" className="h-10 rounded-md border border-input bg-background px-2 text-sm" value={plateUnit} onChange={(event) => { const unit = event.target.value as LoadUnit; setPlateUnit(unit); setPlateBarWeight(unit === "kg" ? "20" : "45"); }}><option value="kg">kg</option><option value="lb">lb</option></select><select aria-label="Barbell weight" className="h-10 rounded-md border border-input bg-background px-2 text-sm" value={plateBarWeight} onChange={(event) => setPlateBarWeight(event.target.value)}>{plateUnit === "kg" ? <><option value="20">20 kg bar</option><option value="15">15 kg bar</option><option value="10">10 kg bar</option></> : <><option value="45">45 lb bar</option><option value="35">35 lb bar</option></>}</select></div><p className="mt-2 text-[10px] text-muted-foreground">Enter the total loaded barbell weight. LyfeOS shows an exact symmetrical layout with common standard plates; confirm the plates available at your location.</p>{plates ? plates.available ? <p className="mt-2 text-xs text-primary">Per side: {plates.perSide.length ? plates.perSide.map((plate) => `${plate.count} × ${plate.weight} ${plates.unit}`).join(" + ") : "bar only"}</p> : <p className="mt-2 text-xs text-muted-foreground">{plates.reason === "below_bar" ? "That target is below the selected bar weight." : plates.reason === "no_exact_standard_layout" ? "No exact layout uses this standard plate set. Choose a different total or confirm nonstandard plates." : "Enter a valid positive total and bar weight."}</p> : null}</details>
    ...
    <div className="space-y-4 mt-4">{exercises.map((exercise, exerciseIndex) => <div key={exerciseIndex} className="rounded-lg border border-muted/20 bg-background/20 p-3">
      ...
      <p className="mt-2 text-[11px] text-muted-foreground">Use the same link group, such as A, on exercises you intentionally alternate. It records the structure you performed; it does not prescribe a routine.</p>{exercise.name.trim() ? (() => { const record = observedRecordFor(exercise.name.trim(), exercise.sets[0]?.loadUnit || preferredLoadUnit); return record ? <p className="mt-2 text-[11px] text-muted-foreground" aria-label={`All-time observed load for ${exercise.name}`}>All-time observed: <span className="font-mono text-primary">{record.bestObservedLoad} {record.loadUnit}</span> · {new Date(record.observedLoadAt).toLocaleDateString()} · source set #{record.observedLoadSetId}. Reference only; LyfeOS does not prefill or prescribe a load.</p> : null; })() : null}
      <div className="space-y-2 mt-2">{exercise.sets.map((set, setIndex) => <div key={setIndex} className="rounded-md border border-muted/20 p-2">
        <div className="grid gap-2 sm:grid-cols-[5rem_7rem_5rem_8rem_auto_auto_auto_auto] sm:items-end">
          <label className="grid gap-1 text-[10px] text-muted-foreground"><span>Reps</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} repetitions`} type="number" min="1" placeholder="Reps" value={set.reps} onChange={(event) => editSet(exerciseIndex, setIndex, "reps", event.target.value)} /></label>
          <label className="grid gap-1 text-[10px] text-muted-foreground"><span>Load ({set.loadUnit})</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} load ${set.loadUnit}`} type="number" min="0" step="0.5" placeholder={`Load ${set.loadUnit}`} value={set.load} onChange={(event) => editSet(exerciseIndex, setIndex, "load", event.target.value)} /></label>
          <label className="grid gap-1 text-[10px] text-muted-foreground"><span>RPE</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} perceived exertion`} type="number" min="1" max="10" placeholder="Optional" value={set.rpe} onChange={(event) => editSet(exerciseIndex, setIndex, "rpe", event.target.value)} /></label>
          <label className="grid gap-1 text-[10px] text-muted-foreground"><span>Set type</span><select aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} kind`} className="h-10 rounded-md border border-input bg-background px-2 text-xs text-foreground" value={set.setKind} onChange={(event) => editSet(exerciseIndex, setIndex, "setKind", event.target.value as SetKind)}><option value="warmup">Warm-up</option><option value="working">Working</option><option value="drop">Drop set</option></select></label>
        ...
        <details className="mt-2"><summary className="cursor-pointer text-[11px] text-muted-foreground">More set details: distance, duration, RIR, and note</summary><div className="mt-2 grid gap-2 sm:grid-cols-[8rem_8rem_6rem_minmax(0,1fr)]"><label className="grid gap-1 text-[10px] text-muted-foreground"><span>Distance (m)</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} distance meters`} type="number" min="1" placeholder="Optional" value={set.distance} onChange={(event) => editSet(exerciseIndex, setIndex, "distance", event.target.value)} /></label><label className="grid gap-1 text-[10px] text-muted-foreground"><span>Duration (min)</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} duration minutes`} type="number" min="1" placeholder="Optional" value={set.duration} onChange={(event) => editSet(exerciseIndex, setIndex, "duration", event.target.value)} /></label><label className="grid gap-1 text-[10px] text-muted-foreground"><span>RIR</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} repetitions in reserve`} type="number" min="0" max="20" placeholder="Optional" value={set.rir} onChange={(event) => editSet(exerciseIndex, setIndex, "rir", event.target.value)} /></label><label className="grid gap-1 text-[10px] text-muted-foreground"><span>Set note</span><Input aria-label={`Exercise ${exerciseIndex + 1} set ${setIndex + 1} note`} placeholder="Optional context" value={set.note} onChange={(event) => editSet(exerciseIndex, setIndex, "note", event.target.value)} /></label></div></details>
      </div>)}</div>
    ...
    <Button className="mt-2 ml-2" data-testid="workout-save" disabled={!activityType.trim() || save.isPending || (editingWorkoutId != null && editingWorkoutRevision == null)} onClick={() => save.mutate()}>{editingWorkoutId ? <Pencil /> : <Plus />}{editingWorkoutId ? "Update workout" : sessionState === "not_started" ? "Log workout" : "Finish & log workout"}</Button>
    ...
    {workouts.data?.workouts.length ? <div className="mt-4 space-y-2">{workouts.data.workouts.map((workout) => <div key={workout.id} className="flex items-center justify-between gap-2 rounded-lg border border-muted/20 bg-background/20 px-3 py-2 text-sm"><span>{workout.activityType}{workout.exercises.length ? ` · ${workout.exercises.map((exercise) => `${exercise.name}${exercise.supersetGroup ? ` [${exercise.supersetGroup}]` : ""} ${exercise.setRecords.map(formatSet).join(" / ")}`).join(", ")}` : ""}</span><div className="flex items-center gap-1"><span className="font-mono text-muted-foreground">{workout.durationMinutes ? `${workout.durationMinutes}m` : "duration not logged"}{workout.perceivedExertion ? ` · RPE ${workout.perceivedExertion}` : ""}</span><Button variant="ghost" size="icon" aria-label={`Edit ${workout.activityType} workout`} disabled={loadHistoricalWorkout.isPending} onClick={() => loadHistoricalWorkout.mutate(workout.id)}><Pencil className="h-4 w-4" /></Button><Button variant="ghost" size="icon" aria-label={`Delete ${workout.activityType} workout`} disabled={removeWorkout.isPending} onClick={() => removeWorkout.mutate(workout.id)}><Trash2 className="h-4 w-4" /></Button></div></div>)}</div> : null}
