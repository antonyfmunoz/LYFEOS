...
type SelectedFood = { name: string; catalogProviderId: string | null; catalogDatasetVersion: string | null; catalogItemVersion: string | null; catalogAttributionText: string | null; catalogAttributionUrl: string | null; catalogEvidence: CatalogEvidence | null; catalogSourceModified: boolean; portions: Array<CatalogPortion & { source: string; sourceModified: boolean; catalogLabel: string | null; catalogGramsPerUnit: number | null }> };
...
export default function FoodCatalogSearch({ onImported, selectedFood }: { onImported: (foodId: number) => void; selectedFood?: SelectedFood }) {
  ...
  const selectedProvider = status.data?.providers?.find((provider) => provider.id === (providerId || status.data?.defaultProviderId));
  ...
  return <div className="rounded-xl border border-primary/15 bg-background/25 p-4" aria-labelledby="food-catalog-heading">
    ...
    <p className="mt-1 text-xs text-muted-foreground">Search the configured data provider. Results remain external until you explicitly save a private copy; provider and dataset attribution stay attached.</p>
    {selectedFood?.catalogProviderId ? <p className="mt-2 rounded-md border border-primary/15 p-2 text-[11px] text-muted-foreground">Selected private food “{selectedFood.name}” came from {selectedFood.catalogProviderId} dataset {selectedFood.catalogDatasetVersion}, item {selectedFood.catalogItemVersion}{selectedFood.catalogSourceModified ? "; its values or portions were privately corrected after import" : ""}. {selectedFood.catalogEvidence ? `${catalogEvidenceLabel(selectedFood.catalogEvidence)} · ${selectedFood.catalogEvidence.reportedCoreNutrientKeys.length}/8 core nutrients were reported when imported.` : "This legacy import has no evidence snapshot."} {selectedFood.portions.filter((portion) => portion.source.startsWith("catalog:")).length} source-attributed portions are saved. {selectedFood.catalogAttributionUrl ? <a className="text-primary underline" href={selectedFood.catalogAttributionUrl} target="_blank" rel="noreferrer">Provider attribution</a> : selectedFood.catalogAttributionText}</p> : null}
    ...
    </div><p className="mt-2 text-[11px] text-muted-foreground" data-testid="food-catalog-coverage-disclosure">{selectedProvider ? `${selectedProvider.name} declares coverage for ${selectedProvider.territories.join(", ")}.` : "Choose a configured source to see its declared coverage."} Coverage varies by product and territory; an unknown result means this source did not identify that barcode, not that the food is unsafe or absent everywhere.</p></> : <p className="mt-3 rounded-md border border-muted/20 p-2 text-xs text-muted-foreground">{status.data?.reason || "Checking catalog availability…"} Manual foods remain available.</p>}
