...
import type { SpreadsheetChart, SpreadsheetChartAxisMode, SpreadsheetChartKind, SpreadsheetDocument } from "@shared/spreadsheets";
...
export function SpreadsheetChartCard({ document, chart, onUpdate, onRemove }: {
  ...
  onUpdate: (patch: { title?: string; kind?: SpreadsheetChartKind; axisMode?: SpreadsheetChartAxisMode }) => void;
  onRemove: () => void;
}) {
  ...
  const visual = chart.kind === "stacked_bar"
    ...
    : chart.kind === "area"
      ...
      : chart.kind === "combo"
        ? <ComposedChart data={chartRows}><CartesianGrid strokeDasharray="3 3" opacity={0.25} /><XAxis dataKey="label" interval="preserveStartEnd" minTickGap={18} /><YAxis yAxisId="primary" width={56} /><YAxis yAxisId="secondary" orientation="right" width={56} hide={!usesDualAxes} /><Tooltip /><Legend />{comboBarSeries?.validCount ? <Bar yAxisId="primary" dataKey={comboBarSeries.key} name={`${comboBarSeries.name} (bar${usesDualAxes ? ", left axis" : ""})`} fill={seriesColors[0]} /> : null}{comboLineSeries.filter((series) => series.validCount > 0).map((series, index) => <Line key={series.key} yAxisId={usesDualAxes ? "secondary" : "primary"} type="monotone" dataKey={series.key} name={`${series.name} (line${usesDualAxes ? ", right axis" : ""})`} stroke={seriesColors[(index + 1) % seriesColors.length]} strokeWidth={2} connectNulls={false} dot={{ r: 3 }} />)}</ComposedChart>
      : chart.kind === "pie"
  ...
  return <article data-testid={`sheet-chart-${chart.id}`} data-chart-kind={chart.kind} data-axis-mode={chart.axisMode} data-source-range={data.sourceRange} className="rounded-xl border border-primary/15 bg-card/30 p-4">
    ...
    {(chart.kind === "pie" || chart.kind === "scatter" || chart.kind === "stacked_bar" || chart.kind === "combo") && <p className="mt-1 text-xs text-muted-foreground">{chart.kind === "pie" ? "Pie charts require one label column and one value column." : chart.kind === "scatter" ? "Scatter charts require one label column followed by exactly two numeric series columns; only complete pairs become points." : chart.kind === "stacked_bar" ? "Stacked bars preserve source-column order and stack every recorded series on the same scale; missing segments stay missing." : usesDualAxes ? "Combination charts use the first source series as bars on the left axis and subsequent line series on the independently scaled right axis. Dual axes can exaggerate visual relationships; compare the disclosed scales and canonical raw-value table." : "Combination charts use the first source series as bars and each subsequent series as lines on one shared scale; source-column order defines these roles."}</p>}
