...
import { resolveSpreadsheetChartSeriesRoles, type SpreadsheetChart, type SpreadsheetChartAxisMode, type SpreadsheetChartKind, type SpreadsheetChartSeriesRole, type SpreadsheetDocument } from "@shared/spreadsheets";
...
export function SpreadsheetChartCard({ document, chart, onUpdate, onRemove }: {
  ...
  onUpdate: (patch: { title?: string; kind?: SpreadsheetChartKind; axisMode?: SpreadsheetChartAxisMode; seriesRoles?: SpreadsheetChartSeriesRole[] }) => void;
  onRemove: () => void;
}) {
  ...
  const comboSeriesRoles = resolveSpreadsheetChartSeriesRoles(chart);
  const comboBarSeries = data.series.filter((_, index) => comboSeriesRoles[index] === "bar");
  const comboLineSeries = data.series.filter((_, index) => comboSeriesRoles[index] === "line");
  const usesDualAxes = chart.kind === "combo" && chart.axisMode === "dual";
  const updateSeriesRole = (seriesIndex: number, role: SpreadsheetChartSeriesRole) => {
    const roles = [...comboSeriesRoles];
    const previousRole = roles[seriesIndex];
    roles[seriesIndex] = role;
    if (!roles.includes(previousRole)) {
      const swapIndex = roles.findIndex((candidate, index) => index !== seriesIndex && candidate === role);
      if (swapIndex >= 0) roles[swapIndex] = previousRole;
    }
    onUpdate({ seriesRoles: roles });
  };
  const commitTitle = () => {
  ...
  const visual = chart.kind === "stacked_bar"
    ...
    : chart.kind === "area"
      ...
      : chart.kind === "combo"
        ? <ComposedChart data={chartRows}><CartesianGrid strokeDasharray="3 3" opacity={0.25} /><XAxis dataKey="label" interval="preserveStartEnd" minTickGap={18} /><YAxis yAxisId="primary" width={56} /><YAxis yAxisId="secondary" orientation="right" width={56} hide={!usesDualAxes} /><Tooltip /><Legend />{comboBarSeries.filter((series) => series.validCount > 0).map((series) => { const index = data.series.indexOf(series); return <Bar key={series.key} yAxisId="primary" dataKey={series.key} name={`${series.name} (bar${usesDualAxes ? ", left axis" : ""})`} fill={seriesColors[index % seriesColors.length]} />; })}{comboLineSeries.filter((series) => series.validCount > 0).map((series) => { const index = data.series.indexOf(series); return <Line key={series.key} yAxisId={usesDualAxes ? "secondary" : "primary"} type="monotone" dataKey={series.key} name={`${series.name} (line${usesDualAxes ? ", right axis" : ""})`} stroke={seriesColors[index % seriesColors.length]} strokeWidth={2} connectNulls={false} dot={{ r: 3 }} />; })}</ComposedChart>
      : chart.kind === "pie"
  ...
  return <article data-testid={`sheet-chart-${chart.id}`} data-chart-kind={chart.kind} data-axis-mode={chart.axisMode} data-series-roles={comboSeriesRoles.join(",")} data-source-range={data.sourceRange} className="rounded-xl border border-primary/15 bg-card/30 p-4">
    <div className="flex flex-wrap items-center gap-2">
      ...
      {chart.kind === "combo" && <Select value={chart.axisMode} onValueChange={(axisMode) => onUpdate({ axisMode: axisMode as SpreadsheetChartAxisMode })}>
        <SelectTrigger aria-label="Combination chart axes" className="w-44"><SelectValue /></SelectTrigger>
        <SelectContent><SelectItem value="shared">Shared scale</SelectItem><SelectItem value="dual">Dual axes</SelectItem></SelectContent>
      </Select>}
    ...
    {chart.kind === "combo" && <details data-testid={`sheet-chart-series-roles-${chart.id}`} className="mt-3 rounded border border-primary/10 p-2">
      <summary className="cursor-pointer text-xs font-medium">Combination series roles</summary>
      <div className="mt-2 grid gap-2 sm:grid-cols-2">
        {data.series.map((series, index) => <label key={series.key} className="space-y-1 text-xs text-muted-foreground"><span>{series.name}</span><Select value={comboSeriesRoles[index]} onValueChange={(role) => updateSeriesRole(index, role as SpreadsheetChartSeriesRole)}><SelectTrigger aria-label={`Series role: ${series.name}`} className="h-8"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="bar">Bar</SelectItem><SelectItem value="line">Line</SelectItem></SelectContent></Select></label>)}
      </div>
      <p className="mt-2 text-[11px] leading-relaxed text-muted-foreground">Every source series has one explicit rendering role. Changing the last series of one role swaps another series so the combination always keeps at least one bar and one line.</p>
    </details>}
    ...
    {(chart.kind === "pie" || chart.kind === "scatter" || chart.kind === "stacked_bar" || chart.kind === "combo") && <p className="mt-1 text-xs text-muted-foreground">{chart.kind === "pie" ? "Pie charts require one label column and one value column." : chart.kind === "scatter" ? "Scatter charts require one label column followed by exactly two numeric series columns; only complete pairs become points." : chart.kind === "stacked_bar" ? "Stacked bars preserve source-column order and stack every recorded series on the same scale; missing segments stay missing." : usesDualAxes ? "Combination charts render each explicitly assigned bar series on the left axis and each line series on the independently scaled right axis. Dual axes can exaggerate visual relationships; compare the disclosed scales and canonical raw-value table." : "Combination charts render every source series with its explicitly assigned bar or line role on one shared scale."}</p>}
