...
export function WorkspaceColumnEditor({ currentDatabaseId, column, definition, databases, canRemove, onChange, onRemove }: Props) {
  ...
  const changeType = (type: WorkspaceColumn["type"]) => {
    ...
    if (type === "formula") base.formula = { expression: "0", resultType: "number" };
    if (type === "rollup") {
  ...
  return <div className="rounded border border-primary/10 p-2 space-y-2">
    ...
    {column.type === "formula" ? <div className="space-y-2">
      <div className="grid gap-2 md:grid-cols-[160px_1fr]">
        <label className="space-y-1 text-xs text-muted-foreground"><span>Result type</span><select aria-label={`${column.name} formula result type`} value={column.formula?.resultType || "number"} onChange={(event) => onChange({ formula: { expression: column.formula?.expression || "0", resultType: event.target.value as NonNullable<WorkspaceColumn["formula"]>["resultType"] } })} className="h-9 w-full rounded-md border border-primary/20 bg-background px-2 text-sm">{(["number", "text", "boolean", "date"] as const).map((resultType) => <option key={resultType}>{resultType}</option>)}</select></label>
        <label className="space-y-1 text-xs text-muted-foreground"><span>Expression</span><Input aria-label={`${column.name} formula`} value={column.formula?.expression || ""} maxLength={300} onChange={(event) => onChange({ formula: { expression: event.target.value, resultType: column.formula?.resultType || "number" } })} placeholder={column.formula?.resultType === "text" ? 'CONCAT([first], " ", [last])' : column.formula?.resultType === "boolean" ? "[score] >= 10" : column.formula?.resultType === "date" ? "ADD_DAYS([start], 7)" : "ROUND([weight] * [reps], 1)"} /></label>
      </div>
      <p className="text-[11px] text-muted-foreground">Use stored scalar/formula IDs, quoted text, TRUE/FALSE, comparisons, arithmetic, and the bounded functions IF, COALESCE, CONCAT, LOWER, UPPER, LENGTH, IS_EMPTY, AND, OR, NOT, ABS, ROUND, MIN, MAX, DAYS_BETWEEN, and ADD_DAYS. Available: {definition.columns.filter((candidate) => candidate.id !== column.id && (storedScalar(candidate) || candidate.type === "formula")).map((candidate) => `[${candidate.id}]`).join(", ") || "none"}.</p>
    </div> : null}
