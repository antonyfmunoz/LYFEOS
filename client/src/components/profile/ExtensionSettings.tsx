...
export default function ExtensionSettings() {
  ...
  return <div className="mb-4 rounded-lg border border-primary/10 bg-background/40 p-4">
    <div className="flex items-start gap-2"><PackageCheck className="mt-0.5 h-4 w-4 text-primary" /><div><h3 className="text-sm font-medium text-foreground">Verified extensions</h3><p className="mt-1 text-xs text-muted-foreground">Install only immutable packages whose publisher signature verifies. Permissions are explicit and immediately revocable.</p></div></div>
    {state.isLoading ? <p className="mt-3 text-xs text-muted-foreground">Loading verified catalog…</p> : null}
    {!state.isLoading && !(state.data?.catalog.length) ? <p className="mt-3 rounded-lg border border-primary/10 bg-card/50 p-3 text-xs text-muted-foreground">No signed extension packages have been published for this installation.</p> : null}
    <div className="mt-3 space-y-2">{state.data?.catalog.map((entry) => { const installation = byPackage.get(entry.id); return <div key={entry.id} className="rounded-lg border border-primary/10 bg-card/50 p-3"><div className="flex items-start justify-between gap-3"><div><p className="text-sm text-foreground">{entry.displayName} <span className="font-mono text-[10px] text-muted-foreground">v{entry.version}</span></p><p className="mt-1 text-xs text-muted-foreground">{entry.description}</p></div><span className="inline-flex items-center gap-1 text-[10px] text-emerald-300"><ShieldCheck className="h-3 w-3" />verified</span></div><p className="mt-2 text-[10px] text-muted-foreground">Publisher: {entry.publisherName} · {entry.manifest.permissions.join(" · ") || "no permissions"}</p>{installation?.status === "enabled" ? <button type="button" disabled={Boolean(busy)} onClick={() => revoke(installation)} className="mt-2 rounded border border-destructive/30 px-2 py-1 text-xs text-destructive disabled:opacity-40">Revoke</button> : <button type="button" disabled={Boolean(busy)} onClick={() => install(entry)} className="mt-2 rounded border border-primary/30 bg-primary/10 px-2 py-1 text-xs text-primary disabled:opacity-40">Review and grant listed permissions</button>}</div>; })}</div>
