  ...
  return (
    <div className="mb-4 rounded-lg border border-primary/10 bg-background/40 p-4">
      ...
      <details className="mt-4" open={!activeWorkspaces.length}>
        ...
        <div className="mt-3 grid gap-2 sm:grid-cols-2"><Input aria-label="Workspace name" className={fieldClass} maxLength={80} placeholder="Workspace name" value={workspaceName} onChange={(event) => setWorkspaceName(event.target.value)} /><Input aria-label="Purpose of working together" className={fieldClass} maxLength={280} placeholder="Purpose of working together" value={workspacePurpose} onChange={(event) => setWorkspacePurpose(event.target.value)} /><Button className="sm:col-span-2" variant="outline" disabled={workspaceName.trim().length < 2 || workspacePurpose.trim().length < 3 || mutate.isPending} onClick={() => mutate.mutate({ url: "/api/collaboration/workspaces", method: "POST", body: { name: workspaceName, purpose: workspacePurpose } })}><Plus className="mr-2 h-4 w-4" />Create workspace</Button></div>
      </details>
