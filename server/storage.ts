...
export interface IStorage {
  ...
  updateUserCategory(id: number, userId: number, data: { value: string; label: string; description?: string }): Promise<UserCategory | null>;
...
export class DatabaseStorage implements IStorage {
  ...
  async getUserByDisplayName(displayName: string): Promise<User | undefined> {
    const [user] = await db.select().from(users).where(sql`lower(${users.displayName}) = lower(${displayName})`);
    return user;
  }
  ...
  async calculateEfficiency(userId: number): Promise<number> {
    ...
    // Component 1: Daily mission completion rate (30% weight)
    ...
    // Component 3: Token utilization (40% weight)
    ...
    // Weighted formula: 30% completion + 30% allocation + 40% utilization
  ...
  async updateUserCategory(id: number, userId: number, data: { value: string; label: string; description?: string }): Promise<UserCategory | null> {
    const setData: Record<string, string> = { value: data.value, label: data.label };
