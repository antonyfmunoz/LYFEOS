...
const migrations = [
  ...
  {
    id: "0123_product_analytics_consent",
    sql: `
  ...
  {
    id: "0124_mission_planning_context_amendments",
    sql: `
  ...
  {
    id: "0125_cross_domain_hypotheses",
    sql: `
  ...
  {
    id: "0126_scheduled_workflow_automations",
    sql: `
  ...
  {
    id: "0127_distributed_request_rate_limits",
    sql: `
  ...
  },
  {
    ...
    sql: `
      ...
      ALTER TABLE "umh_outbox_events" DROP CONSTRAINT IF EXISTS "umh_outbox_events_status_valid";
      ALTER TABLE "umh_outbox_events"
        ADD CONSTRAINT "umh_outbox_events_status_valid"
        CHECK ("status" IN ('pending', 'processing', 'retry', 'delivered', 'failed'));
      ALTER TABLE "umh_outbox_events" DROP CONSTRAINT IF EXISTS "umh_outbox_events_lease_valid";
      ALTER TABLE "umh_outbox_events"
  ...
  {
    id: "0129_personal_finance_foundation",
    sql: `
  ...
  {
    id: "0130_health_provider_credential_vault",
    sql: `
  ...
  {
    id: "0131_integration_provider_credential_vault",
    sql: `
  ...
  {
    id: "0135_installation_branding",
    sql: `
      CREATE TABLE IF NOT EXISTS "lyfeos_installations" (
        "id" text PRIMARY KEY NOT NULL, "product_key" text NOT NULL DEFAULT 'lyfeos', "product_owner" text NOT NULL DEFAULT 'OST', "status" text NOT NULL DEFAULT 'active', "current_brand_revision" integer NOT NULL DEFAULT 1, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "lyfeos_installations_identity_valid" CHECK ("product_key" = 'lyfeos' AND "product_owner" = 'OST'), CONSTRAINT "lyfeos_installations_status_valid" CHECK ("status" IN ('active','suspended','terminated')), CONSTRAINT "lyfeos_installations_revision_positive" CHECK ("current_brand_revision" > 0)
      );
      CREATE TABLE IF NOT EXISTS "installation_brand_revisions" (
        "id" serial PRIMARY KEY NOT NULL, "installation_id" text NOT NULL REFERENCES "lyfeos_installations"("id") ON DELETE CASCADE, "revision" integer NOT NULL, "brand" jsonb NOT NULL, "actor_user_id" integer REFERENCES "users"("id") ON DELETE SET NULL, "reason" text NOT NULL, "created_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "installation_brand_revisions_installation_revision_unique" UNIQUE ("installation_id", "revision"), CONSTRAINT "installation_brand_revisions_revision_positive" CHECK ("revision" > 0)
      );
      CREATE TABLE IF NOT EXISTS "installation_domain_bindings" (
        "id" serial PRIMARY KEY NOT NULL, "installation_id" text NOT NULL REFERENCES "lyfeos_installations"("id") ON DELETE CASCADE, "hostname" text NOT NULL, "status" text NOT NULL DEFAULT 'pending', "verification_token_hash" text, "verified_at" timestamp, "revoked_at" timestamp, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "installation_domain_bindings_status_valid" CHECK ("status" IN ('pending','verified','revoked')), CONSTRAINT "installation_domain_bindings_hostname_valid" CHECK ("hostname" = lower("hostname") AND char_length("hostname") BETWEEN 4 AND 253)
      );
      ...
      CREATE TABLE IF NOT EXISTS "installation_admin_grants" (
        "id" serial PRIMARY KEY NOT NULL, "installation_id" text NOT NULL REFERENCES "lyfeos_installations"("id") ON DELETE CASCADE, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "role" text NOT NULL DEFAULT 'brand_admin', "status" text NOT NULL DEFAULT 'active', "created_at" timestamp NOT NULL DEFAULT now(), "revoked_at" timestamp,
        CONSTRAINT "installation_admin_grants_installation_user_unique" UNIQUE ("installation_id", "user_id"), CONSTRAINT "installation_admin_grants_role_valid" CHECK ("role" IN ('brand_admin','installation_owner')), CONSTRAINT "installation_admin_grants_status_valid" CHECK ("status" IN ('active','revoked'))
      );
      CREATE TABLE IF NOT EXISTS "installation_audit_events" (
        "id" serial PRIMARY KEY NOT NULL, "installation_id" text NOT NULL REFERENCES "lyfeos_installations"("id") ON DELETE CASCADE, "actor_user_id" integer REFERENCES "users"("id") ON DELETE SET NULL, "action" text NOT NULL, "subject_type" text NOT NULL, "subject_id" text, "metadata" jsonb NOT NULL DEFAULT '{}'::jsonb, "created_at" timestamp NOT NULL DEFAULT now()
      );
      ...
      INSERT INTO "installation_brand_revisions" ("installation_id", "revision", "brand", "reason") VALUES ('default', 1, '{"productName":"LyfeOS","shortName":"LyfeOS","accentColor":"#00e0ff","supportUrl":"https://lyfeos.net"}'::jsonb, 'OST default installation') ON CONFLICT ("installation_id", "revision") DO NOTHING;
      INSERT INTO "installation_domain_bindings" ("installation_id", "hostname", "status", "verified_at") VALUES ('default', 'lyfeos.net', 'verified', now()), ('default', 'www.lyfeos.net', 'verified', now()), ('default', 'lyfeos-app.fly.dev', 'verified', now()) ON CONFLICT (lower("hostname")) DO NOTHING;
    `,
  },
  {
    id: "0136_consent_bound_collaboration",
    sql: `
      CREATE TABLE IF NOT EXISTS "collaboration_workspaces" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL, "owner_user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "name" text NOT NULL, "purpose" text NOT NULL, "status" text NOT NULL DEFAULT 'active', "revision" integer NOT NULL DEFAULT 1, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "collaboration_workspaces_name_valid" CHECK (char_length("name") BETWEEN 2 AND 80), CONSTRAINT "collaboration_workspaces_purpose_valid" CHECK (char_length("purpose") BETWEEN 3 AND 280), CONSTRAINT "collaboration_workspaces_status_valid" CHECK ("status" IN ('active','archived')), CONSTRAINT "collaboration_workspaces_revision_valid" CHECK ("revision" > 0)
      );
      ...
      CREATE TABLE IF NOT EXISTS "collaboration_memberships" (
        "id" serial PRIMARY KEY NOT NULL, "workspace_id" uuid NOT NULL REFERENCES "collaboration_workspaces"("id") ON DELETE CASCADE, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "invited_by_user_id" integer REFERENCES "users"("id") ON DELETE SET NULL, "role" text NOT NULL, "status" text NOT NULL DEFAULT 'invited', "invitation_purpose" text NOT NULL, "accepted_at" timestamp, "revoked_at" timestamp, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "collaboration_memberships_workspace_user_unique" UNIQUE ("workspace_id", "user_id"), CONSTRAINT "collaboration_memberships_role_valid" CHECK ("role" IN ('owner','coach','collaborator')), CONSTRAINT "collaboration_memberships_status_valid" CHECK ("status" IN ('invited','active','declined','revoked','left')), CONSTRAINT "collaboration_memberships_purpose_valid" CHECK (char_length("invitation_purpose") BETWEEN 3 AND 280)
      );
      ...
      CREATE TABLE IF NOT EXISTS "collaboration_visibility_grants" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL, "workspace_id" uuid NOT NULL REFERENCES "collaboration_workspaces"("id") ON DELETE CASCADE, "owner_user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "grantee_user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "subject_type" text NOT NULL, "subject_id" integer NOT NULL, "scopes" jsonb NOT NULL DEFAULT '[]'::jsonb, "purpose" text NOT NULL, "status" text NOT NULL DEFAULT 'active', "expires_at" timestamp NOT NULL, "revoked_at" timestamp, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "collaboration_visibility_grants_subject_valid" CHECK ("subject_type" IN ('mission','thread')), CONSTRAINT "collaboration_visibility_grants_status_valid" CHECK ("status" IN ('active','revoked')), CONSTRAINT "collaboration_visibility_grants_subject_id_valid" CHECK ("subject_id" > 0), CONSTRAINT "collaboration_visibility_grants_purpose_valid" CHECK (char_length("purpose") BETWEEN 3 AND 280), CONSTRAINT "collaboration_visibility_grants_not_self" CHECK ("owner_user_id" <> "grantee_user_id")
      );
      ...
      CREATE TABLE IF NOT EXISTS "collaboration_audit_events" (
        "id" serial PRIMARY KEY NOT NULL, "workspace_id" uuid NOT NULL REFERENCES "collaboration_workspaces"("id") ON DELETE CASCADE, "actor_user_id" integer REFERENCES "users"("id") ON DELETE SET NULL, "subject_user_id" integer REFERENCES "users"("id") ON DELETE SET NULL, "action" text NOT NULL, "subject_type" text NOT NULL, "subject_id" text, "metadata" jsonb NOT NULL DEFAULT '{}'::jsonb, "created_at" timestamp NOT NULL DEFAULT now()
      );
  ...
  {
    id: "0137_ai_execution_and_signed_extensions",
    sql: `
      CREATE TABLE IF NOT EXISTS "ai_execution_preferences" (
        "user_id" integer PRIMARY KEY REFERENCES "users"("id") ON DELETE CASCADE, "execution_mode" text NOT NULL DEFAULT 'cloud', "preferred_provider" text NOT NULL DEFAULT 'anthropic', "cloud_fallback_enabled" boolean NOT NULL DEFAULT false, "revision" integer NOT NULL DEFAULT 1, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "ai_execution_preferences_mode_valid" CHECK ("execution_mode" IN ('local','hybrid','cloud')), CONSTRAINT "ai_execution_preferences_provider_valid" CHECK ("preferred_provider" IN ('self_hosted','anthropic')), CONSTRAINT "ai_execution_preferences_revision_valid" CHECK ("revision" > 0), CONSTRAINT "ai_execution_preferences_fallback_valid" CHECK ("execution_mode" = 'hybrid' OR "cloud_fallback_enabled" = false)
      );
      ...
      CREATE TABLE IF NOT EXISTS "extension_publishers" (
        "key_id" text PRIMARY KEY NOT NULL, "name" text NOT NULL, "public_key_pem" text NOT NULL, "status" text NOT NULL DEFAULT 'active', "created_at" timestamp NOT NULL DEFAULT now(), "revoked_at" timestamp,
        CONSTRAINT "extension_publishers_key_id_valid" CHECK ("key_id" ~ '^[a-z0-9][a-z0-9._-]{2,79}$'), CONSTRAINT "extension_publishers_name_valid" CHECK (char_length("name") BETWEEN 2 AND 100), CONSTRAINT "extension_publishers_status_valid" CHECK ("status" IN ('active','revoked'))
      );
      CREATE TABLE IF NOT EXISTS "extension_packages" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL, "slug" text NOT NULL, "version" text NOT NULL, "display_name" text NOT NULL, "description" text NOT NULL, "manifest" jsonb NOT NULL, "manifest_digest" text NOT NULL, "publisher_key_id" text NOT NULL REFERENCES "extension_publishers"("key_id") ON DELETE RESTRICT, "signature" text NOT NULL, "status" text NOT NULL DEFAULT 'published', "created_at" timestamp NOT NULL DEFAULT now(), "revoked_at" timestamp,
        CONSTRAINT "extension_packages_slug_version_unique" UNIQUE ("slug", "version"), CONSTRAINT "extension_packages_slug_valid" CHECK ("slug" ~ '^[a-z0-9][a-z0-9-]{2,63}$'), CONSTRAINT "extension_packages_version_valid" CHECK ("version" ~ '^[0-9]+\\.[0-9]+\\.[0-9]+$'), CONSTRAINT "extension_packages_display_name_valid" CHECK (char_length("display_name") BETWEEN 2 AND 100), CONSTRAINT "extension_packages_description_valid" CHECK (char_length("description") BETWEEN 3 AND 500), CONSTRAINT "extension_packages_digest_valid" CHECK ("manifest_digest" ~ '^sha256:[0-9a-f]{64}$'), CONSTRAINT "extension_packages_status_valid" CHECK ("status" IN ('published','revoked'))
      );
      ...
      CREATE TABLE IF NOT EXISTS "extension_installations" (
        "id" uuid PRIMARY KEY DEFAULT gen_random_uuid() NOT NULL, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "package_id" uuid NOT NULL REFERENCES "extension_packages"("id") ON DELETE RESTRICT, "extension_slug" text NOT NULL, "granted_permissions" jsonb NOT NULL DEFAULT '[]'::jsonb, "status" text NOT NULL DEFAULT 'enabled', "revision" integer NOT NULL DEFAULT 1, "installed_at" timestamp NOT NULL DEFAULT now(), "revoked_at" timestamp, "updated_at" timestamp NOT NULL DEFAULT now(),
        CONSTRAINT "extension_installations_user_package_unique" UNIQUE ("user_id", "package_id"), CONSTRAINT "extension_installations_status_valid" CHECK ("status" IN ('enabled','revoked')), CONSTRAINT "extension_installations_revision_valid" CHECK ("revision" > 0)
      );
      ...
      CREATE TABLE IF NOT EXISTS "extension_audit_events" (
        "id" serial PRIMARY KEY NOT NULL, "user_id" integer REFERENCES "users"("id") ON DELETE CASCADE, "package_id" uuid REFERENCES "extension_packages"("id") ON DELETE SET NULL, "action" text NOT NULL, "metadata" jsonb NOT NULL DEFAULT '{}'::jsonb, "created_at" timestamp NOT NULL DEFAULT now()
      );
  ...
  {
    id: "0141_registration_disclosure_provenance",
    sql: `
      ALTER TABLE "users"
  ...
  },
  {
    ...
    sql: `
      ...
      CREATE INDEX IF NOT EXISTS "canvas_templates_user_updated_idx" ON "canvas_templates" ("user_id", "updated_at" DESC);
    `,
  },
  {
    id: "0143_cross_product_consent_lifecycle",
    sql: `
      ALTER TABLE "cross_product_sharing_preferences" ADD COLUMN IF NOT EXISTS "revision" integer NOT NULL DEFAULT 1;
      ALTER TABLE "cross_product_sharing_preferences" ADD COLUMN IF NOT EXISTS "federation_subject_id" uuid NOT NULL DEFAULT gen_random_uuid();
      ALTER TABLE "cross_product_sharing_preferences" DROP CONSTRAINT IF EXISTS "cross_product_sharing_preferences_federation_subject_id_unique";
      ALTER TABLE "cross_product_sharing_preferences" ADD CONSTRAINT "cross_product_sharing_preferences_federation_subject_id_unique" UNIQUE ("federation_subject_id");
      ALTER TABLE "cross_product_sharing_preferences" DROP CONSTRAINT IF EXISTS "cross_product_sharing_preferences_revision_valid";
      ALTER TABLE "cross_product_sharing_preferences" ADD CONSTRAINT "cross_product_sharing_preferences_revision_valid" CHECK ("revision" > 0);
      ...
      CREATE INDEX IF NOT EXISTS "cross_product_sharing_revisions_user_created_idx" ON "cross_product_sharing_revisions" ("user_id", "created_at" DESC, "id" DESC);
      INSERT INTO "cross_product_sharing_revisions" ("user_id", "revision", "state", "allowed_destinations", "allowed_purposes", "affected_destinations", "affected_purposes", "policy_version", "delivery_state", "created_at")
      SELECT "user_id", "revision", CASE WHEN "ecosystem_sharing_enabled" THEN 'enabled' ELSE 'disabled' END,
        ...
        "allowed_destinations", "allowed_purposes", 'lyfeos.cross-product-sharing.v1', 'not_configured', "updated_at"
      FROM "cross_product_sharing_preferences"
      ON CONFLICT ("user_id", "revision") DO NOTHING;
    `,
  },
  {
    id: "0144_integration_action_governance",
    sql: `
      ...
      CREATE INDEX IF NOT EXISTS "integration_action_receipts_user_created_idx" ON "integration_action_receipts" ("user_id", "created_at");
      CREATE INDEX IF NOT EXISTS "integration_action_receipts_pending_idx" ON "integration_action_receipts" ("user_id", "state", "expires_at");
    `,
  },
  {
    id: "0145_mission_external_links",
    sql: `
      ...
      CREATE INDEX IF NOT EXISTS "mission_external_links_user_quest_idx" ON "mission_external_links" ("user_id", "quest_id");
  ...
  {
    id: "0147_brand_ownership_review",
    sql: `
  ...
  {
    id: "0148_food_review_preferences",
    sql: `
  ...
  {
    id: "0149_food_package_confirmations",
    sql: `
  ...
  {
      id: "0156_grocery_recall_monitoring",
      sql: `
  ...
  {
    id: "0158_ingredient_evidence_classifications",
    sql: `
      ...
      ALTER TABLE "ingredient_scan_items" ADD CONSTRAINT "ingredient_scan_items_classification_valid"
        CHECK ("classification" IN ('unknown', 'label_fact', 'preference_match', 'regulatory_notice', 'declared_major_allergen_label_term', 'declared_color_additive', 'declared_sulfiting_agent', 'declared_non_nutritive_sweetener', 'declared_caffeine_source', 'declared_partially_hydrogenated_oil'));
      ALTER TABLE "ingredient_scan_items" DROP CONSTRAINT IF EXISTS "ingredient_scan_items_evidence_strength_valid";
      ALTER TABLE "ingredient_scan_items" ADD CONSTRAINT "ingredient_scan_items_evidence_strength_valid"
  ...
  {
    id: "0163_data_vault_workspace_completion",
    sql: `
      CREATE TABLE IF NOT EXISTS "slides" ("id" serial PRIMARY KEY, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "title" text NOT NULL, "description" text, "document" jsonb NOT NULL, "favorite" boolean NOT NULL DEFAULT false, "deleted_at" timestamp, "revision" integer NOT NULL DEFAULT 1, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now());
      ...
      CREATE TABLE IF NOT EXISTS "slide_revisions" ("id" serial PRIMARY KEY, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "slide_id" integer NOT NULL REFERENCES "slides"("id") ON DELETE CASCADE, "revision_number" integer NOT NULL, "action" text NOT NULL, "snapshot" jsonb NOT NULL, "created_at" timestamp NOT NULL DEFAULT now(), CONSTRAINT "slide_revisions_slide_revision_idx" UNIQUE ("slide_id", "revision_number"));
      CREATE INDEX IF NOT EXISTS "slide_revisions_user_slide_created_idx" ON "slide_revisions" ("user_id", "slide_id", "created_at");
      CREATE TABLE IF NOT EXISTS "vault_forms" ("id" serial PRIMARY KEY, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "title" text NOT NULL, "description" text, "definition" jsonb NOT NULL, "confirmation_text" text NOT NULL DEFAULT 'Response saved.', "backing_database_id" integer REFERENCES "workspace_databases"("id") ON DELETE SET NULL, "favorite" boolean NOT NULL DEFAULT false, "active" boolean NOT NULL DEFAULT true, "deleted_at" timestamp, "created_at" timestamp NOT NULL DEFAULT now(), "updated_at" timestamp NOT NULL DEFAULT now());
      ...
      CREATE TABLE IF NOT EXISTS "vault_form_responses" ("id" serial PRIMARY KEY, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "form_id" integer NOT NULL REFERENCES "vault_forms"("id") ON DELETE CASCADE, "values" jsonb NOT NULL, "created_at" timestamp NOT NULL DEFAULT now());
      CREATE INDEX IF NOT EXISTS "vault_form_responses_user_form_created_idx" ON "vault_form_responses" ("user_id", "form_id", "created_at");
      CREATE TABLE IF NOT EXISTS "vault_item_states" ("id" serial PRIMARY KEY, "user_id" integer NOT NULL REFERENCES "users"("id") ON DELETE CASCADE, "item_type" text NOT NULL, "item_id" integer NOT NULL, "favorite" boolean NOT NULL DEFAULT false, "trashed_at" timestamp, "last_opened_at" timestamp, "updated_at" timestamp NOT NULL DEFAULT now(), CONSTRAINT "vault_item_states_user_type_item_idx" UNIQUE ("user_id", "item_type", "item_id"));
