...
const requiredHeaders = ["source_record_id", "metric_key", "display_name", "category", "value", "unit", "observed_at"] as const;
...
const date = (value: string | undefined) => { const raw = value?.trim() || ""; if (!/^\d{4}-\d{2}-\d{2}T/.test(raw)) return null; const result = new Date(raw); return Number.isNaN(result.getTime()) ? null : result; };

export function previewHealthObservationCsv(csvText: string): HealthCsvPreview {
  const rows = cells(csvText); if (rows.length < 2) throw new Error("The CSV needs a header and at least one data row.");
  const headers = rows[0].map((value, index) => (index === 0 ? value.replace(/^\uFEFF/, "") : value).trim().toLowerCase().replaceAll(" ", "_"));
  ...
  const previewRows = rows.slice(1, 201).map((row, offset) => {
    ...
    const sourceRecordId = text(at(row, "source_record_id"), 200); const metricKey = text(at(row, "metric_key"), 80); const displayName = text(at(row, "display_name"), 120); const category = text(at(row, "category"), 40); const value = number(at(row, "value")); const unit = text(at(row, "unit"), 32); const observedAt = date(at(row, "observed_at"));
  ...
  return { importHash: createHash("sha256").update(csvText).digest("hex"), rows: previewRows, validCount: previewRows.filter((row) => row.entry).length, invalidCount: previewRows.filter((row) => !row.entry).length, disclosure: "LyfeOS keeps only the rows you explicitly import. It does not retain the CSV file, infer missing values, or create a provider connection." };
}
