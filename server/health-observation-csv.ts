...
export type ImportedObservation = { source: "lab" | "imported"; sourceRecordId: string; metricKey: string; displayName: string; category: string; value: number; unit: string; observedAt: Date; method: string | null; methodVersion: string | null; deviceName: string | null; labName: string | null; specimenType: string | null; collectedAt: Date | null; referenceLow: number | null; referenceHigh: number | null; referenceUnit: string | null; note: string | null };
...
export function previewHealthObservationCsv(csvText: string, options: HealthCsvImportOptions = {}): HealthCsvPreview {
  ...
  return { importHash: createHash("sha256").update(JSON.stringify({ csvText, defaultCategory: options.defaultCategory || null })).digest("hex"), rows: previewRows, validCount: previewRows.filter((row) => row.entry).length, invalidCount: previewRows.filter((row) => !row.entry).length, disclosure: `LyfeOS keeps only the rows you explicitly import. It does not retain the CSV file, invent missing measurements, or create a provider connection. Lab-category rows remain attributed as lab records; other rows remain attributed as imported records.${sourceIdIsPanelAccession ? " Accession-number rows use the reported accession plus normalized test key as their stable row identity." : ""} If you selected a fallback category, that is shown in this preview and bound to the reviewed import.` };
}
