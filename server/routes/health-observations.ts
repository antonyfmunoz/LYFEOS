...
export function registerHealthObservationRoutes(app: Express): void {
  ...
  app.post("/api/health-observations/imports/commit", isAuthenticated, async (req: Request, res: Response) => {
    ...
    const userId = req.session.userId!; const existing = await db.select({ sourceRecordId: healthObservations.sourceRecordId }).from(healthObservations).where(and(eq(healthObservations.userId, userId), inArray(healthObservations.source, ["lab", "imported"]), inArray(healthObservations.sourceRecordId, sourceRecordIds)));
