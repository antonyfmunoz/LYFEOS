...
const accountExportTables = [
  ...
  "finance_accounts", "finance_balance_snapshots", "finance_transactions", "finance_budgets", "finance_goals", "spirit_entries",
...
export async function deleteLocalAccountData(userId: number): Promise<void> {
  await db.transaction(async (tx) => {
    ...
    // until PostHog accepts the asynchronous event-deletion request.
    await tx.execute(sql`
      INSERT INTO "product_analytics_deletion_queue" ("subject_id")
      SELECT DISTINCT "subject_id" FROM "product_analytics_consents" WHERE "user_id" = ${userId}
    ...
    await tx.execute(sql`DELETE FROM "mission_evidence_provider_bindings" WHERE "user_id" = ${userId}`);
    ...
    for (const table of [
      ...
      "finance_balance_snapshots", "finance_transactions", "finance_budgets", "finance_goals", "finance_accounts", "spirit_entries",
