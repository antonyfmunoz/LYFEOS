...
import { z } from "zod";
import { brandOwnershipAvailability, lookupBrandOwnership } from "../brand-ownership";
...
export function registerBrandOwnershipRoutes(app: Express): void {
  ...
  app.post("/api/brand-ownership/lookup", isAuthenticated, (req: Request, res: Response) => {
    const parsed = lookupSchema.safeParse(req.body);
    if (!parsed.success) return res.status(400).json({ error: "Enter a valid brand before checking ownership." });
    return res.json(lookupBrandOwnership(parsed.data.brand));
  });
}
