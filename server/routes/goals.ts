...
export function registerGoalRoutes(app: Express): void {
  ...
  // Stripe payment routes (stubbed - Stripe not currently connected)
  ...
  app.get("/api/stripe/subscription", isAuthenticated, async (_req: Request, res: Response) => {
    res.json({ subscription: null });
  });

  app.post("/api/stripe/checkout", isAuthenticated, async (_req: Request, res: Response) => {
    res.status(503).json({ error: "Payment processing is not currently configured" });
  });

  app.post("/api/stripe/portal", isAuthenticated, async (_req: Request, res: Response) => {
    res.status(503).json({ error: "Payment processing is not currently configured" });
  });
